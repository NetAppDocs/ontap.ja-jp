...
[role="lead"]
ONTAP 9 .10.1以降では、ラーニングモードのAutonomous Ransomware Protection（ARP；自律型ランサムウェア対策）で新しいボリュームがデフォルトで有効になるようにStorage VM（SVM）を設定できます。
...
ARPは、設定の変更後にSVMで作成されたボリュームでのみ有効になります。既存のボリュームでは、ARPは有効になりません。方法をご確認くださいlink:enable-task.html["既存のボリュームでARPを有効にします"]。

ONTAP 9 13.1以降、ARP分析にアダプティブラーニングが追加され、ラーニングモードからアクティブモードへの切り替えが自動的に行われます。詳細については、を参照してください link:index.html#learning-and-active-modes["学習モードとアクティブモード"]。
...
+
CLIでは、新しいボリュームに対してデフォルトでARPを有効にして新しいSVMを作成することもできます。
