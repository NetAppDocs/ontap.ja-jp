...
ONTAP CLIでは、を使用できます `security anti-ransomware volume workload-behavior show` 現在までに検出されたファイル拡張子を表示するコマンド。ただし、このツールを使用して学習期間を短縮することはお勧めしません。
...
ONTAP CLIでは、この機能を管理するための新しいコマンドファミリーが導入されています。 `security anti-ransomware volume`。を使用することもできます `volume modify` コマンドにを指定します `-anti-ransomware` フィーチャーを管理するパラメーター。
