...
.このタスクについて
ARPは、常に最初にドライラン（ラーニングモード）状態でイネーブルにする必要があります。アクティブな状態から始めた場合、過剰な誤検出レポートが生成される可能性があります。

ARPをラーニングモードで最低30日間実行することをお勧めします。ONTAP 9.13.1以降では、ARPによって最適な学習期間間隔が自動的に決定され、30日前にスイッチが自動化されます。

詳細については、を参照してください link:index.html#learning-and-active-mode["学習およびアクティブモード"]。
