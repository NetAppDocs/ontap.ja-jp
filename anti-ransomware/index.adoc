...
[role="lead"]
ONTAP 9.10.1以降、ONTAP管理者は自律型ランサムウェア対策（ARP）を有効にすることで、NAS（NFSおよびSMB）環境におけるワークロード分析を実行し、ランサムウェア攻撃を示唆する可能性のある異常なアクティビティを事前に検知して警告することができます。ARPはONTAPに直接組み込まれており、ONTAPの他の機能との統合された制御と調整を確保します。ARPはリアルタイムで動作し、ファイルシステムへの書き込みや読み取り時にデータを処理し、潜在的なランサムウェア攻撃を迅速に検知して対応します。

ARPは、スケジュールされたSnapshotに加えて、定期的にロックされたSnapshotを作成します。Snapshotの保持期間は動的に調整されます：異常なアクティビティが検出されない場合はSnapshotは迅速に再利用され、攻撃が検出される前に取得されたSnapshotは調査と復旧のために長期間保持されます。

プロトコルのサポート、ワークロードの適合性、およびバージョン固有の要件については、link:use-cases-restrictions-concept.html["ARPのユースケースと考慮事項"]を参照してください。

ARPによって生成されたスナップショットの詳細（ONTAPバージョンによって追加された変更を含む）については、<<ARPスナップショット>>を参照してください。
...
ONTAP 9.18.1以降、ARPはデフォルトで、AFF AシリーズおよびAFF Cシリーズ、ASA、ASA r2システムのすべての新しいボリュームで自動的に有効になります。この自動デフォルトARP有効化はlink:use-cases-restrictions-concept.html#unsupported-configurations["サポートされていないボリュームまたは構成"]には適用されません。既存のボリュームでは<<manual-default-enablement,ARPを手動で有効にする>>する必要があります。アップグレード時にデフォルトの有効化がどのように適用されるかの詳細については、link:use-cases-restrictions-concept.html#arp-upgrade-considerations["ARPバージョン固有のアップグレードに関する考慮事項"]を参照してください。


NOTE: ONTAP 9.19.1以降、デフォルトのARP有効化は、SnapMirror同期およびSnapMirrorアクティブ同期SANの関係を含むクラスタに適用されます。ONTAP 9.18.1では、これらの構成に対してデフォルトの有効化が抑制されていました。クラスタ間のSnapMirror同期関係の場合、デフォルトの有効化は、ソースクラスタと宛先クラスタの両方がONTAP 9.19.1にアップグレードされると有効になります。AFXシステムでは、SnapMirror同期サポートがある場合でも、ARPはデフォルトで有効になっていません。
...
評価
 a| 
暗号化の基本動作を判断するために、2～4週間の評価期間が設けられます。ベースラインの閾値が設定されている間にも、検出と警告が発生する可能性があります。評価期間が完了したかどうかは、 `security anti-ransomware volume show`コマンドを実行して `Block device detection status`を確認することで判断できます。

link:respond-san-entropy-eval-period.html["ブロックデバイスワークロードに対するONTAP ARP/AI評価期間について学ぶ"]。
 a| 
ONTAP 9.17.1P5以降でONTAPが検出したハイパーバイザー仮想ディスクを含むNASボリューム
...
.ARPスナップショットのバージョンによる違い
* *保持設定を変更する*：ONTAP 9.11.1以降では、link:modify-automatic-snapshot-options-task.html["Snapshotのデータ保持設定を変更する"]できます。
* *間隔と異常応答*：ONTAP 9.17.1以降、NASボリュームとSANボリュームの両方について、定期的にARPスナップショットが生成されます。また、検出された異常に応じて、ARPスナップショットも生成されます。これは常に標準的な対応でした。ONTAPはARPスナップショットの先頭に名前を追加して、識別しやすくします。
* *SnapMirror同期とアクティブ同期*：ONTAP 9.19.1以降、SnapMirror同期またはSnapMirrorアクティブ同期の関係に参加するボリューム用に作成されたARPスナップショットには、以下の追加ルールが適用されます。
+
** ARPスナップショットはプライマリボリューム上に作成され、保持されます。
** ARPスナップショットは、SnapMirror同期またはSnapMirrorアクティブ同期の一部としてセカンダリに複製されません。フェイルオーバー後、リカバリボリュームでlink:enable-task.html["ARPを再度有効にする"]する必要があります。





NOTE: SnapMirror SynchronousまたはSnapMirror Active Sync SANボリュームでのボリュームレベルのリストア(`volume snapshot restore`）では、SnapMirror SynchronousまたはSnapMirror Active Sync関係を一時的に休止または解除する必要がある場合があります。多くの場合、プライマリボリューム上のARPまたはその他のSnapshotからFlexCloneやファイルレベルのリストア処理を使用することで、SnapMirror SynchronousまたはSnapMirror Active Syncの解除を回避できます。

link:use-cases-restrictions-concept.html#snapmirror["SnapMirror同期およびSnapMirrorアクティブ同期のサポートの詳細については、こちらをご覧ください"]。
