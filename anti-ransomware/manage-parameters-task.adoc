...
ボリュームやワークロードによっては、異なる検出パラメータが必要になることがあります。たとえば、ARP 対応ボリュームにはさまざまな種類のファイル拡張子がホストされている可能性があります。その場合、これまでに見たことのないファイル拡張子のしきい値数をデフォルトの 20 より大きい数に変更するか、これまでに見たことのないファイル拡張子に基づく警告を無効にする必要があります。ONTAP 9.11.1以降では、特定のワークロードに合わせて攻撃検出パラメータを変更できます。
