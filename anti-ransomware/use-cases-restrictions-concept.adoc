...
* ARP/AI 分析は、SnapMirror同期およびSnapMirrorアクティブ同期SAN関係のアクティブな読み書きボリューム上で実行されます。ARPはプライマリボリューム上にスナップショットを作成し、機械学習に基づくランサムウェア検出を実行します。
* ARPスナップショットは、SnapMirror同期またはSnapMirrorアクティブ同期関係のセカンダリ ボリュームにはレプリケートされません。フェイルオーバー後、リカバリボリュームでlink:enable-task.html["ARPを再度有効にする"]必要があります。
