...
* ONTAP 9.19.1 RC にアップグレードし、SnapMirror 同期または SnapMirror アクティブ同期関係の一部であるボリュームで ARP を有効にすると、関係が一時的に同期がずれた状態になることがあります。この状態は一時的なものであり、介入なしに自動的に関係が再同期されるはずです。
