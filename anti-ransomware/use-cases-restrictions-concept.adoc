...
* ONTAP 9.10.1から9.15.1では、4,000種類以上の固有のファイル拡張子を持つボリューム上のNASワークロード。
+
オリジナルのARPモデルは、ボリュームごとに最大4,000種類のファイル拡張子を分析できます。ボリューム上の固有ファイル拡張子の数がこの制限を超えると、ARPはランサムウェアの活動を効率的に検出できなくなり、ワークロードはARPに適さないと判断されます。この制限は、ONTAP 9.16.1以降のARP/AIには適用されません。
...
| ARP/AI のデフォルトの有効化^2^ | ✓ | ✓ |  |  |  |  |  |  |  |  
|===
^1^ ONTAP 9.16.1および9.17.1はFlexGroupボリュームのARP/AIサポートを提供しません。ONTAP 9.18.1以降、FlexGroupボリュームはARP/AIモデルを使用します。これらのバージョンにアップグレードする際の動作については、<<arp-upgrade-considerations,ARPバージョン固有のアップグレードに関する考慮事項>>を参照してください。

^2^ ONTAP 9.18.1へのアップグレード後のARPデフォルト有効化動作に関する情報については、<<arp-upgrade-considerations,ARPバージョン固有のアップグレードに関する考慮事項>>を参照してください。

^3^ ONTAP 9.19.1以降、ARP/AIはFAS、AFF A-series、AFF C-series、およびAFXシステムのNASとSANのSnapMirror同期関係のプライマリボリューム、およびAFF A-series、AFF C-series、ASA r2システムのSnapMirrorアクティブ同期SAN関係のプライマリボリュームをサポートします。詳細については、<<SnapMirror>>を参照してください。
...
* ARP構成状態とARPスナップショットは、ONTAP 9.19.1のSnapMirror同期またはSnapMirrorアクティブ同期の一部としてセカンダリ ボリュームにレプリケートされません。
...
== ARPバージョン固有のアップグレードに関する考慮事項

既にARPを使用しているONTAPクラスターをアップグレードする場合は、link:../upgrade/special-considerations.html["解決する必要があるONTAPの特別な考慮事項"]に加えて、アップグレードによって生じるARP固有の動作変更に注意する必要があります。



[[arp-default-enablement-after-upgrading-to-ontap-9-18-1]]
=== ONTAP 9.18.1へのアップグレード後のARPデフォルト有効化

ONTAP 9.18.1以降、ARP/AIのデフォルト有効化動作は、AFF Aシリーズ、AFF Cシリーズ、ASA、およびASA r2システムで利用可能です。アップグレード後、12時間の猶予期間を経て、すべての新規ボリュームでARP/AIが自動的に有効になります。既存のボリュームではlink:index.html#manual-default-enablement["ARPを手動で有効にする"]する必要があります。



[[snapmirror-relationships-after-upgrading-to-ontap-9-19-1]]
=== SnapMirror ONTAP 9.19.1 へのアップグレード後の関係

アップグレード後にSnapMirror同期またはSnapMirrorアクティブ同期関係の一部であるボリュームでARPを有効にすると、関係が一時的に同期解除状態に移行します。この状態は一時的なものであり、介入なしに自動的に関係が再同期されるはずです。



[[flexgroup-volumes-after-upgrade]]
=== アップグレード後のFlexGroupボリューム

ONTAP 9.16.1および9.17.1は、FlexGroupボリュームに対するARP/AIサポートを提供しません。これらのバージョンにアップグレードした後、ARPが有効になっているFlexGroupボリュームは、ARP/AI導入以前に使用されていたものと同じARPモデルで引き続き動作します。ONTAP 9.18.1以降、FlexGroupボリュームはARP/AIモデルを使用します。
