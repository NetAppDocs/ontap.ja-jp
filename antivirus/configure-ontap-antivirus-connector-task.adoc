...
. NetAppサーバ用証明書への署名に使用されたCA証明書（およびルートCAまでの中間署名局）がPrivacy Enhanced Mail（PEM）形式であり、その名前が「Ontap_CA.pem」であることを確認します。この証明書をAntivirus Connectorのインストール ディレクトリに配置します。ONTAPシステムで、「ONTAP」のウイルス対策コネクタのクライアント証明書に署名するために使用される CA 証明書（およびルート CA までの中間署名機関）を「client-ca」タイプの証明書としてインストールします。
