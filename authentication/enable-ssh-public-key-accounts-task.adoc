...
[role="lead"]
を使用できます `security login create` 管理者アカウントがSSH公開鍵を使用して管理またはデータSVMにアクセスできるようにするコマンド。
...
+
次のコマンドは、SVM管理者アカウントを有効にします `svmadmin1` 事前定義されたを使用する `vsadmin-volume` SVMにアクセスするためのロール``engData1`` SSH公開鍵を使用して、次の手順を実行
