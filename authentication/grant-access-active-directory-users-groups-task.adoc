...
[role="lead"]
を使用できます `security login create` コマンドを使用して、Active Directory（AD）のユーザアカウントまたはグループアカウントが管理またはデータSVMにアクセスできるようにします。AD グループのすべてのユーザは、グループに割り当てられたロールを使用して SVM にアクセスできます。
...
[NOTE]
====
ADグループアカウントによるアクセスは、でのみサポートされます `SSH` および `ontapi` アプリケーション：
...
+
次のコマンドは、ADクラスタ管理者アカウントを有効にします `DOMAIN1\guest1` 事前定義されたを使用する `backup` 管理SVMにアクセスするためのロール``engCluster``。
...
----
+
次のコマンドは、ADグループアカウントのSVM管理者アカウントを有効にします `DOMAIN1\adgroup` 事前定義されたを使用する `vsadmin-volume` SVMにアクセスするためのロール``engData``。
