...
summary: ONTAP 9.14 以降では、Open Authorization （OAuth 2.0）フレームワークを使用して ONTAP クラスターへのアクセスを制御するオプションが追加されました。この機能は、ONTAP CLI、System Manager、ONTAP REST API などの ONTAP 管理インターフェイスを使用して設定できます。ただし、OAuth 2.0 の認証とアクセス制御の決定は、クライアントが ONTAP REST API を使用して ONTAP にアクセスする場合にのみ適用されます。 
---
...
[role="lead"]
ONTAP 9.14 以降では、Open Authorization （OAuth 2.0）フレームワークを使用して ONTAP クラスターへのアクセスを制御するオプションが追加されました。この機能は、ONTAP CLI、System Manager、ONTAP REST API などの ONTAP 管理インターフェイスを使用して設定できます。ただし、OAuth 2.0 の認証とアクセス制御の決定は、クライアントが ONTAP REST API を使用して ONTAP にアクセスする場合にのみ適用されます。
