...
[role="lead"]
ONTAP のマニュアルページでは、ONTAP CLIコマンドの使用方法について説明しています。マニュアルページは、ONTAP コマンドラインでを使用して参照できます `man` コマンドを実行します
...
限定サポートのバージョンのONTAP のコマンドリファレンスはPDFとして提供されています。
...
のCLI比較ツールを使用すると、ONTAP リリース間でのコマンドラインインターフェイス（CLI）コマンドの変更点を確認できます link:https://mysupport.netapp.com/site/info/cli-comparison["NetApp Support Site"^]。
