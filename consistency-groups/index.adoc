...
システム マネージャの整合性グループ メニューの「概要」タブで監視データを表示するか、ONTAP REST API でリクエストして表示することができます。ONTAP 9.14.1 以降では、 `consistency-group metrics show` コマンドを使用して ONTAP CLI で整合性グループのメトリクスを表示できます。 `consistency-group metrics show` の詳細については、link:https://docs.netapp.com/us-en/ontap-cli/vserver-consistency-group-metrics-show.html["ONTAPコマンド リファレンス"^] を参照してください。


NOTE: ONTAP 9.13.1 では、履歴メトリックは ONTAP REST API を使用してのみ取得できます。ONTAP 9.14.1 以降、履歴メトリックは System Manager でも利用できます。
...
整合性グループは、アプリケーション整合性のある保護を提供し、複数のボリュームまたはLIF間でデータの一貫性を確保します。整合性グループのスナップショットを作成する際、整合性グループ上に「フェンス」が設定されます。フェンスはスナップショット操作が完了するまでI/Oのキューを開始し、整合性グループ内のすべてのエンティティ間でデータのポイントインタイム整合性を確保します。フェンスは、スケジュールされたスナップショットポリシーの実行やSystem Managerを使用したスナップショットの作成など、スナップショット作成操作中に一時的なレイテンシの急増を引き起こす可能性があります。ONTAP REST APIおよびONTAP CLIのコンテキストの詳細については、link:https://kb.netapp.com/on-prem/ontap/DM/REST-API["ONTAP REST APIドキュメント"^]およびlink:https://docs.netapp.com/us-en/ontap-cli/["ONTAPコマンド リファレンス"^]を参照してください。
...
ミラーリングされたアグリゲート上に配置された整合性グループに関連付けられたボリュームとスナップショットは、リモートサイト（サイト B）に複製されます。サイト B 上のボリュームの内容は、整合性グループに対する書き込み順序保証を提供するため、災害発生時にサイト B から復旧することが可能です。ONTAP 9.11.1 以降を実行しているクラスターの ONTAP REST API および System Manager で整合性グループを使用して、整合性グループのスナップショットにアクセスできます。ONTAP 9.14.1 以降では、ONTAP CLI を使用してスナップショットにアクセスすることもできます。
...
ONTAP REST API で作成された整合グループ スナップショットは、System Manager の整合グループ インターフェイスおよび整合グループ ONTAP REST API エンドポイントを通じて管理できます。ONTAP 9.14.1 以降、整合グループ スナップショットは ONTAP CLI でも管理できます。
