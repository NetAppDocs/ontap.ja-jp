...
* スナップショット作成を実行する際に、5秒から120秒の間でタイムアウト値を任意に設定できます。タイムアウト値が指定されていない場合、操作はデフォルト値の7秒でタイムアウトします。ONTAP REST API では、 `action_timeout`パラメータを使用してタイムアウト値を設定します。ONTAP CLI では、 `-timeout`フラグを使用します。


.手順
2段階のスナップショットは、ONTAP REST API を使用するか、ONTAP 9.14.1 以降では ONTAP CLI を使用して実行できます。この操作は System Manager ではサポートされていません。


NOTE: ONTAP REST API を使用してスナップショットの作成を呼び出す場合は、ONTAP REST API を使用してスナップショットをコミットする必要があります。ONTAP CLI を使用してスナップショットの作成を呼び出す場合は、ONTAP CLI を使用してスナップショットをコミットする必要があります。方法を混在させることはサポートされていません。
