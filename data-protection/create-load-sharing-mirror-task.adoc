...
+
ルートボリュームとデスティネーションボリュームには、などのサフィックスを付けることを推奨します `_root` および `_m1`。
...
+
次の例は、ルートボリューム間に負荷共有ミラー関係を作成します `svm1_root` および負荷共有ミラーボリュームが含まれます `svm1_m1`：
...
----
+
負荷共有ミラーのtype属性がから変わります `DP` 終了： `LS`。
