...
[role="lead"]
ONTAP 9.3以降では、 `snapmirror protect`データ保護関係を 1 つのステップで構成するコマンド。レプリケートするボリュームのリスト、宛先クラスタ上の SVM、ジョブ スケジュール、およびSnapMirrorポリシーを指定します。 `snapmirror protect`残りはやります。
...
ONTAP 9.4 以降では、宛先ボリュームに最大 1019 個のスナップショットを含めることができます。  ONTAP 9.3 以前では、宛先ボリュームには最大 251 個のスナップショットを含めることができます。
...
詳細はこちら `snapmirror protect`の中でlink:https://docs.netapp.com/us-en/ontap-cli/snapmirror-protect.html["ONTAPコマンド リファレンス"^] 。
