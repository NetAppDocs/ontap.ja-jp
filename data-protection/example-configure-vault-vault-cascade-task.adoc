...
[role="lead"]
レプリケーション関係を一度に 1 ステップずつ設定する方法の具体例を示します。この例で設定したヴォールト-ヴォールトカスケード構成を使用すると、「my-weekly」というラベルの付いた251個を超えるSnapshotを保持できます。
...
* SnapMirrorラベルが「`my-daily'」、「`my-weekly'」、および「`my-monthly'」のSnapshotをソースクラスタで設定しておきます。
...
* 「SnapVault _ secondary」ポリシーは、セカンダリデスティネーションクラスタに7個の日単位Snapshot、52個の週単位Snapshot、180個の月単位Snapshotを保持します。
* 「SnapVault _ tertiaryポリシー」は、3番目のデスティネーションクラスタに250個の週単位Snapshotを保持します。
