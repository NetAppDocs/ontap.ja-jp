...
[role="lead"]
を使用できます `-identity-preserve false` のオプション `snapmirror create` コマンドは、SVMのボリュームとセキュリティ設定のみをレプリケートします。一部のプロトコルとネームサービスの設定も保持されます。
...
====
+
次の例は、デフォルトを使用して、SnapMirror DR関係を作成します `MirrorAllSnapshots` ポリシー：この関係では、ネットワーク、ネームサービス、およびその他の設定が SVM レプリケーションから除外されます。
...
----
+
次の例は、デフォルトを使用してユニファイドレプリケーション関係を作成します `MirrorAndVault` ポリシー：この関係では、ネットワーク、ネームサービス、およびその他の設定が除外されます。
...
----
+
ポリシータイプがのカスタムポリシーを作成している場合 `async-mirror`、次の例は、SnapMirror DR関係を作成します。この関係では、ネットワーク、ネームサービス、およびその他の設定が SVM レプリケーションから除外されます。
...
----
+
ポリシータイプがのカスタムポリシーを作成している場合 `mirror-vault`次の例は、ユニファイドレプリケーション関係を作成します。この関係では、ネットワーク、ネームサービス、およびその他の設定が SVM レプリケーションから除外されます。
