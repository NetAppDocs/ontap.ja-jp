...
.このタスクについて
ONTAP 9.11.1以降では、を使用して、災害復旧のリハーサル中に再同期時間を短縮できます `-quick-resync true` のオプション `snapmirror resync` コマンドを実行している間にSVM DR関係の逆再同期を実行する。迅速な再同期により、Data Warehouseの再構築およびリストア処理をバイパスすることで、本番環境に戻るまでの時間を短縮できます。
...
====
+
次の例は、元のデスティネーションSVM svm_backupと元のソースSVMの間の反転した関係を解放します。 `svm1`
