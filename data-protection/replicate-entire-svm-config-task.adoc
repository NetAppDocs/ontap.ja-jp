...
[role="lead"]
を使用できます `-identity-preserve true` のオプション `snapmirror create` コマンドを使用して、SVMの設定全体をレプリケートできます。
...
ONTAP 9.9..1以降では、ミラーバックアップポリシーを使用する場合に、ソースとデスティネーションのSVMで別 々 のSnapshotポリシーを作成でき、デスティネーションのSnapshotコピーがソースのSnapshotコピーで上書きされなくなります。詳細については、を参照してください link:snapmirror-svm-replication-concept.html["SnapMirror SVM レプリケーションの概要"]。
...
====
+
次の例は、デフォルトを使用して、SnapMirror DR関係を作成します `MirrorAllSnapshots` ポリシー：
...
----
+
ポリシータイプがのカスタムポリシーを作成している場合 `async-mirror`を使用して、SnapMirror DR関係を作成する例を次に示します。
...
----
+
ポリシータイプがのカスタムポリシーを作成している場合 `mirror-vault`に、ユニファイドレプリケーション関係を作成する例を示します。
