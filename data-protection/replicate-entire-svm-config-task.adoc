...
* ONTAP 9.19.1 以降、SVM DR 用に構成された SVM では IPsec の有効化はサポートされていません。IPsec を使用している場合は、無効にするか、別の SVM を選択して SVM DR 関係を構成する必要があります。ONTAP 9.19.1 以降を使用していて、SVM DR 用に構成された SVM で IPsec を有効にしようとすると、エラーが発生します。
...
. ソースクラスターから、 `vserver peer create` コマンドを使用して SVM のピア関係を作成します。
+
