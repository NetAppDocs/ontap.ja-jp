...
[role="lead"]
を使用できます `volume snapshot partial-restore-file` データの開始バイトオフセットとバイト数を把握している場合は、SnapshotコピーからLUNまたはNFS / SMBコンテナファイルに一定範囲のデータをリストアするコマンドです。このコマンドでは、同じ LUN 内に複数のデータベースを格納するホスト上のいずれかのデータベースをリストアできます。
...
+
次の例は、ファイルの最初の4、096バイトをリストアします `myfile.txt`：
