...
* ONTAP 9.13.1 以降、 ONTAP は再同期時間を短縮するために、デフォルトでクイック再同期を使用するようになりました。クイック再同期をデフォルトで使用するには、次の条件を満たしている必要があります。
+
** FlexVolボリュームにはクローンがありません
** MirrorAllSnapshotsポリシーを使用する場合
+

NOTE: 使用 `-quick-resync` 転送されたデータ ブロックのストレージ効率が削除されるため、再同期先のボリュームで追加のスペースが消費される可能性があります。この追加のスペース消費は、再同期先のインラインまたはレプリケーション後のストレージ効率アプリケーションの一部として回復されます。

+
その `-quick-resync` パラメータはオプションです。クイック再同期を有効または無効にするには、 `-quick-resync true|false` パラメータを `snapmirror resync` 指示。

+
詳細については `-quick-resync`、を参照してください link:https://docs.netapp.com/us-en/ontap-cli/snapmirror-resync.html["ONTAPコマンド リファレンス"^]。
