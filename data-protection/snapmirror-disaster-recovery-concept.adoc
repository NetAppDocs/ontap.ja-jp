...
* ONTAP 9.13.1以降では、非同期SnapMirrorを使用してを保護できます xref:../consistency-groups/index.html[整合グループ]。ONTAP 9.14.1以降では、非同期SnapMirrorを使用して、整合性グループ関係を使用してボリューム単位のSnapshotをデスティネーションクラスタにレプリケートできます。詳細については、を参照してください xref:../consistency-groups/protect-task.html#configure-snapmirror-asynchronous[非同期SnapMirror保護を設定する]。
