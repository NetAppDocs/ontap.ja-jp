...
+
ONTAP 9.9.9..1以降では、ミラーバックアップポリシーを使用する場合の動作が変更されています。ONTAP 9.9.9..1以降では、ソースとデスティネーションに異なるSnapshotポリシーを作成でき、デスティネーションのSnapshotコピーがソースのSnapshotコピーで上書きされません。
...
+
。 `-identity-preserve false` オプションを使用すると、SVMのボリューム、認証および許可の設定、およびに示すプロトコルとネームサービスの設定のみがレプリケートされます link:snapmirror-svm-replication-concept.html#configurations-replicated-in-svm-dr-relationships["SVM DR 関係でレプリケートされる設定"]。

* 。 `-discard-configs network` のオプション `snapmirror policy create` ソースとデスティネーションのSVMが異なるサブネットにある場合に使用するため、コマンドではSVMレプリケーションからLIFと関連ネットワークの設定が除外されます。
* 。 `-vserver-dr-protection unprotected` のオプション `volume modify` コマンドは、指定したボリュームをSVMレプリケーション対象から除外します。
...
次の表は、の連動を示しています `snapmirror create `-identity-preserve` オプションと `snapmirror policy create` -discard-configs networkオプション：
...
|  |  | *なしのポリシー `-discard-configs network` セット* | *との方針 `-discard-configs network` セット* |  
