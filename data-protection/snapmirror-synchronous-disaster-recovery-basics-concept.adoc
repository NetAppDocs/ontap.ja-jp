...
| 自律型ランサムウェア対策 | ONTAP 9.19.1 | アップグレード後にSnapMirror同期またはSnapMirrorアクティブ同期関係の一部であるボリュームでARPを有効にすると、関係が一時的に同期解除状態に移行します。この状態は一時的なものであり、介入なしに自動的に関係が再同期されるはずです。詳細については、link:../anti-ransomware/use-cases-restrictions-concept.html["自律型ランサムウェア対策のユースケースと考慮事項"]を参照してください。 
