...
| アプリケーションで作成されたSnapshotのレプリケーション | ONTAP 9.7 |  `snapshot create`処理時にスナップショットに適切なラベルが付けられている場合、CLIまたはONTAP APIを使用して、SnapMirror同期は、アプリケーションを休止した後、ユーザが作成したスナップショットと外部スクリプトで作成されたスナップショットの両方をレプリケートします。ONTAP 9.14.1以降、アプリケーションが作成したスナップショットのレプリケーションにおけるラベル要件はなくなりました。アプリケーションが作成したスナップショットのレプリケートの詳細については、link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Protection_and_Security/SnapMirror/How_to_replicate_application_created_snapshots_with_SnapMirror_Synchronous["NetAppナレッジベース：アプリケーションが作成したスナップショットをSnapMirror同期的に複製する方法"^]を参照してください。 


| 自律型ランサムウェア対策 | ONTAP 9.19.1 |  
...
| NDMP / NDMPのリストア | ONTAP 9.13.1 | ソースとデスティネーション クラスタの両方が ONTAP 9.13.1 以降を実行している必要があります。これは、SnapMirror 同期ボリュームのダンプまたは SMTape ベースの NDMP バックアップを実行するためです。ダンプベースまたは SMTape ベースの NDMP リストアは、SnapMirror アクティブ同期または SnapMirror 同期関係にあるデスティネーション ボリュームではサポートされていません。 
...
| Snapshot管理操作のレプリケーション | ONTAP 9.19.1 | ONTAP 9.19.1以降、スナップショット管理操作 `snapshot delete`、 `snapshot rename`、および `snapshot modify`はセカンダリクラスタにレプリケートされます。 


| スケジュールされたスナップショットのレプリケーション | ONTAP 9.19.1 | ONTAP 9.19.1以降では、SnapMirrorポリシーを使用して、スケジュールされたスナップショットのレプリケーションを有効にできます。ONTAP 9.19.1より前のリリースでは、スナップショットポリシーを使用して作成されたスケジュール済みスナップショットはレプリケートされません。 
...
| SVMディザスタ リカバリ | ONTAP 9.6  a| 
* SnapMirror 同期ソースは、SVM ディザスタリカバリソースにもなり得ます。たとえば、SnapMirror 同期を一方の経路とし、SVM ディザスタリカバリをもう一方の経路とするファンアウト構成などがあります。
* SnapMirror同期ではデータ保護ソースのカスケードがサポートされないため、SnapMirror同期のソースをSVMディザスタ リカバリのデスティネーションにすることはできません。デスティネーション クラスタでSVMディザスタ リカバリの逆再同期を実行する前に、同期関係を解放する必要があります。
* SVMディザスタ リカバリはDPボリュームのレプリケーションをサポートしていないため、SnapMirror同期のデスティネーションをSVMディザスタ リカバリのソースにすることはできません。同期ソースの逆再同期を行うと、SVMディザスタ リカバリでデスティネーション クラスタのDPボリュームが除外されます。
...
* SnapCenter
* SnapLockボリューム
* SnapMirror アクティブ同期または SnapMirror 同期関係にあるデスティネーション ボリュームへのダンプまたは SMTape ベースの NDMP リストア
...
ONTAP 9.19.1以降、 `snapmirror initialize`すべてのスナップショットをセカンダリクラスタに複製しますが、 `snapmirror resync`最後の共通スナップショット以降に作成された新しいスナップショットのみを複製します。再同期操作中、関係ステータスが `InSync`のときにプライマリのみとしてタグ付けされた非レプリケートスナップショットはセカンダリクラスタに複製されず、これらのスナップショットはフリップ再同期操作後に新しいセカンダリクラスタに保持されます。


NOTE: SnapMirror同期ボリュームでスナップショットを作成すると、ユーザーI/Oレイテンシが増加する可能性があります。30分を超える間隔でスナップショットスケジュールを使用することを推奨します。

.関連情報
* https://www.netapp.com/pdf.html?item=/media/17174-tr4733.pdf["NetAppテクニカル レポート4733：SnapMirror同期構成とベストプラクティス"^]
