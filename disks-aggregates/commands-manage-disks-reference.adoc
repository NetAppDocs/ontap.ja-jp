...
summary: ディスクを管理するには、storage disk コマンドと storage aggregate コマンドを使用できます。 
---
= ディスクを管理するためのONTAPコマンド
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
 `storage disk`および `storage aggregate`コマンドを使用してディスクを管理できます。
...
| 状況 | 使用するコマンド 
 ...
 a| 
スペア ディスクの一覧を表示する（所有者別のパーティショニングされたディスクなど）
 a| 
`storage aggregate show-spare-disks`
 ...
 a| 
ディスクのRAIDタイプ、現在の使用状況、およびRAIDグループをローカル層別に表示します
 a| 
`storage aggregate show-status`
 ...
 a| 
物理ディスクのRAIDタイプ、現在の使用状況、ローカル層、スペアを含むRAIDグループを表示します
 a| 
...
ディスクのクラスタ構成前の（nodescope）ドライブ名を表示する
 a| 
`storage disk show -primary-paths`（上級）
...
共有ディスクのパーティショニングを解除する
 a| 
`storage disk unpartition`（診断レベルで利用可能）
