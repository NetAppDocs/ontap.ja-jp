...
summary: Flash Pool ローカル階層を作成するには、HDD RAID グループで構成された既存のローカル階層でこの機能を有効にしてから、そのローカル階層に 1 つ以上の SSD RAID グループを追加します。これにより、そのローカル階層には 2 セットの RAID グループが作成されます。SSD RAID グループ（SSD キャッシュ）と HDD RAID グループです。 
---
