...
+
ドライブ（データ、パリティ、ダブルパリティ）のRAIDタイプはで確認できます `Position` 列（Column）：状況に応じて `Position` 列が表示されます `shared`を指定すると、ドライブが共有されます。HDDの場合は、パーティショニングされたディスクです。SSDの場合は、ストレージプールの一部です。
