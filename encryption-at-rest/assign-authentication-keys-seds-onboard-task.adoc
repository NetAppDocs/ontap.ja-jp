...
[role="lead"]
を使用できます `storage encryption disk modify` コマンドを使用して、FIPSドライブまたはSEDにデータ認証キーを割り当てます。このキーは、クラスタノードでドライブのデータにアクセスする際に使用します。
...
[NOTE]
====
を使用できます `security key-manager key query -key-type NSE-AK` キーIDを表示するためのコマンド。
