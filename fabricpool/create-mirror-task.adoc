...
.手順
. 既存のFabricPool設定を使用しない場合は、コマンドを使用してオブジェクトストアをアグリゲートに接続して新しい設定を作成し `storage aggregate object-store attach`ます。
+
...
----
. コマンドを使用して、2つ目のオブジェクトストアをアグリゲートに接続し `storage aggregate object-store mirror`ます。
+
