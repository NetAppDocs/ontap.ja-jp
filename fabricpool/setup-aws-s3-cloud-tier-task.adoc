...
* ONTAP がAWS S3オブジェクトサーバとの接続に使用するLIFは10Gbpsポート上に配置することを推奨します。
...
+
** を使用します `-auth-type` `CAP` C2Sにアクセスするためのクレデンシャルを取得するためのパラメータ。
+
を使用する場合 `-auth-type` `CAP` パラメータを使用する場合は、を使用する必要があります `-cap-url` C2Sにアクセスするための一時的なクレデンシャルを要求するための完全なURLを指定するパラメータ。
...
+
。 `storage aggregate object-store config modify` コマンドを使用して、FabricPool のAWS S3の設定情報を変更できます。
