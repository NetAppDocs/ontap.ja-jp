...
+
** 。 `storage aggregate object-store config create` 指定された情報でローカルONTAP システムがONTAP S3サーバにアクセスできない場合、コマンドは失敗します。
...
** を設定します `-is-certificate-validation-enabled` パラメータの値 `false` ONTAP S3の証明書チェックを無効にします。
