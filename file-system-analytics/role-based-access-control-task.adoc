...
[role="lead"]
ONTAP 9.12.1 以降、ONTAP には `admin-no-fsa`と呼ばれる定義済みのロールベースアクセス制御（RBAC）ロールが含まれています。 `admin-no-fsa`ロールは管理者レベルの権限を付与しますが、ONTAP CLI、ONTAP REST API、および System Manager における `files`エンドポイント（ファイルシステム分析）に関連する操作をユーザーが実行できないようにします。
