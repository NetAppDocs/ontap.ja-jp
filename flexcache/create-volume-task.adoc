...
* ONTAP 9.18.1 以降、 FlexCache は、SVM-DR 関係に属する SVM を使用して元のボリュームのキャッシュ ボリュームを作成することをサポートします。
+
ONTAP 9.18.1 以降を実行している場合、ストレージ管理者は、SVM-DR 関係の一部である元のボリュームのキャッシュ ボリュームを作成する前に、SVM-DR 関係の一部であるプライマリおよびセカンダリの元の SVM の両方とキャッシュ SVM をピアリングする必要があります。
