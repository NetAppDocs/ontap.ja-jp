...
プライマリ SVM には FlexCache オリジン ボリュームを配置できますが、セカンダリ SVM には配置できません。FlexCache 関係はレプリケートされませんが、オリジン ボリュームのデータは通常のボリュームとしてレプリケートされます。
...
NOTE: 9.5より前のONTAP 9リリースでは、元のFlexVolからデータを提供できる対象が、Data ONTAP 7-Mode 8.2.xを実行するシステムで作成されたFlexCacheボリュームに限定されていました。ONTAP 9.5以降では、ONTAP 9システムで作成されたFlexCacheボリュームにもデータを提供できます。7-Mode FlexCacheからONTAP 9 FlexCacheへの移行については、link:https://www.netapp.com/pdf.html?item=/media/7336-tr4743.pdf["NetAppテクニカルレポート4743：FlexCache in ONTAP"^]を参照してください。
