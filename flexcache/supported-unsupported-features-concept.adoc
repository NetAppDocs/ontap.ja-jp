...
NOTE: FabricPool階層化が有効になっている元のボリュームのキャッシュとしてFlexCacheボリュームを作成することはできますが、 FlexCacheボリューム自体を階層化することはできません。
...
ONTAP 9.5 以降でサポートされます。SVM DR 関係のプライマリ SVM には元のボリュームを含めることができますが、 ONTAP 9.18.1 より前のONTAPリリースを実行している場合は、SVM DR 関係が解除されたときに、新しい元のボリュームを使用してFlexCache関係を再作成する必要があります。

ONTAP 9.18.1 以降では、元の SVM がフェイルオーバーすると、キャッシュは DR サイトの元の SVM に自動的に切り替わります。手動による回復手順は不要になります。
