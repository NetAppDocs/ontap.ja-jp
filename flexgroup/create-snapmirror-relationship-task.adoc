...
[NOTE]
====
ONTAP 9.3を実行しているソースクラスタとONTAP 9.2以前を実行しているデスティネーションクラスタの間にSnapMirror関係を作成した場合、ソースFlexGroup FlexGroupボリュームにqtreeを作成するとSnapMirrorの更新が失敗します。この状況からリカバリするには、FlexGroupボリューム内のデフォルト以外のqtreeをすべて削除し、FlexGroupボリュームのqtree機能を無効にしてから、qtree機能が有効になっているSnapshotコピーをすべて削除する必要があります。
