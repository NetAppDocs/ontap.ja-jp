...
.. 初期化処理と更新処理の際に転送するSnapshotコピーを決定するSnapVaultポリシーのルールを定義します。 `snapmirror policy add-rule -vserver svm_name -policy policy_for_rule - snapmirror-label snapmirror-label -keep retention_count -schedule schedule`
