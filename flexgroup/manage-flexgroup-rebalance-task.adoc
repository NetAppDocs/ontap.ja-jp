...
+
FlexVolからFlexGroupへの変換後は、 FlexGroup の自動再バランス調整を使用しないことをお勧めします。代わりに、既存のファイルを再配布するには、 `volume rebalance file-move start` ONTAP 9.10.1 以降で使用可能なコマンド。この操作はデフォルトで非中断です(`-is-disruptive false`）。ビジー状態のファイルの一部が移動できない場合は、中断モードでコマンドを再実行できます。(`-is-disruptive true` ) を計画されたメンテナンス時間中に実行します。詳細はこちら `volume rebalance file-move start`の中でlink:https://docs.netapp.com/us-en/ontap-cli/volume-rebalance-file-move-start.html["ONTAPコマンド リファレンス"^] 。
