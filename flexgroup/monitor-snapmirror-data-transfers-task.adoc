...
.手順
. すべてのFlexGroup ボリューム関係のSnapMirror関係ステータスを確認します。 `snapmirror show -relationship-group-type flexgroup`
+
[listing]
----
...
----
. FlexGroup ボリュームの各コンスティチュエントのSnapMirror関係ステータスを確認します。 `snapmirror show -expand`
+
[listing]
----
...
...
----
. SnapMirror転送に失敗した場合は、転送が失敗したFlexGroup ボリュームのコンスティチュエントとエラーの原因を特定します。 `snapmirror show -fields last-transfer-error -expand`
+
[listing]
----
