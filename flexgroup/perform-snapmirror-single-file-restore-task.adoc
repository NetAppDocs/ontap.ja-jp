...
* 単一ファイルのリストア転送に失敗した場合は、を使用してクリーンアップする必要があります `clean-up-failure` のオプション `snapmirror restore` コマンドを実行します
* FlexGroup ボリュームの拡張は、 FlexGroup による単一ファイルのリストアが進行中または中止された状態の場合にサポートされます


.手順
. FlexGroup ボリュームからファイルをリストアします。``snapmirror restore -destination-path destination_path -source-path source_path -file-list /f1 -throttle throttle -source-snapshot snapshot``
+
