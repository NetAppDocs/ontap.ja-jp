...
+
クォータの初期化中にクォータに違反したかどうかを確認するには、を使用します `volume quota report` コマンドを実行します
...
. タイプがのクォータポリシールールを作成する必要があります `user` 現実的な値のディスクのソフトリミットとハードリミットを指定します。
+
[listing]
----
...
. タイプがのクォータポリシールールを作成する必要があります `user`を指定します。この場合、クォータターゲットに複数のユーザ（UNIXユーザ、SMBユーザ、またはその両方）を指定し、現実的な値のディスクのソフトリミットとハードリミットを指定します。
+
[listing]
----
...
. タイプがのクォータポリシールールを作成する必要があります `user`を使用して、クォータターゲットとしてUNIXユーザまたはWindowsユーザを指定します `user-mapping` をに設定します `on`をクリックし、現実的な値のディスクのソフトリミットとハードリミットを指定してルールを作成します。
+
UNIXユーザとWindowsユーザのマッピングは、を使用して事前に設定しておく必要があります `vserver name-mapping create` コマンドを実行します
...
. タイプがのクォータポリシールールを作成する必要があります `tree` また、現実的な値のディスクのソフトリミットとハードリミットがルールに設定されている場合もあります。
+
[listing]
----
...
. NFSクライアントから、を使用します `df` コマンドを使用して、合計スペース使用量、使用可能なスペース、および使用済みスペースを表示します。
+
[listing]
----
...
+
** ONTAP 9.2およびONTAP 9.1では、クォータターゲットのタイプとしてのみを指定できます `user` または `group` FlexGroup の場合。
+
FlexGroup 9.2 および ONTAP 9.1 の ONTAP では、ツリークォータタイプはサポートされません。

** ONTAP 9.3以降では、クォータターゲットのタイプをに指定できます `user`、 `group`または `tree` FlexGroup の場合。
...
FlexGroup ボリュームにと表示される場合があります `mixed` State。まだすべてのコンスティチュエントボリュームの状態が同じではないことを示します。
...
2 entries were displayed.
----
. クォータレポートを表示してアクティブなクォータがあるFlexGroup ボリュームを確認します。 `volume quota report -vserver svm_name -volume flexgroup_vol`
+
