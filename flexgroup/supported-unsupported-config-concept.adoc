...
* SVMを移行
* FabricPool 、FlexGroup 、およびSVM-DRの連携。
+
[NOTE]
====
ONTAP 9.12.1よりも前のリリースでは、これらの機能のいずれか2つが連携して動作していましたが、3つすべてが連携しているわけではありません。
...
* 継続的可用性を備えた SMB 共有
* MetroCluster 構成
* FlexGroup ボリュームの名前を変更する (`volume rename` コマンド）
* FlexGroup ボリュームのサイズを縮小する (`volume size` コマンド）
* エラスティックサイジング
...
* SMB 共有の変更通知の機能拡張
+
変更通知は、が配置されている親ディレクトリに対する変更について送信されます `changenotify` プロパティが設定され、その親ディレクトリ内のすべてのサブディレクトリに対する変更が反映されます。
...
* SMB 共有の変更通知
+
通知は、が置かれている親ディレクトリに対する変更についてのみ送信されます `changenotify` プロパティが設定されています。親ディレクトリのサブディレクトリに対する変更については送信されません。
