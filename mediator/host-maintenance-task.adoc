...
NOTE: HTTPS のみをサポートする ONTAP Mediator 1.12 以降をインストールした場合、 SCST パッケージはインストールされておらず、ホスト OS カーネルは ONTAP Mediator に影響を与えることなくアップグレードできます。アップグレード手順については、ホスト OS のマニュアルを参照してください。
...
.タスク概要
* ONTAP Mediatorをインストールする前にこの手順を実行してください。使用しているSCSTのバージョンがONTAP Mediatorインストーラにバンドルされているバージョンよりも新しい場合、インストーラはこの手順をスキップします。
* 場合によっては、アップグレードされたOSにパッチが必要なため、アップグレードされたOSへのSCSTのデフォルトインストールが失敗することがあります。
+
ONTAP Mediator 1.12.1 以降では、インストールが失敗した場合、 `/usr/local/bin/mediator_repair_scst`にある SCST 修復ツールを使用してインストールを修復できます。

+
アップグレードされた OS にパッチが必要な場合、ツールには ONTAP Mediator パッケージに同梱されている SCST のバージョン用のパッチが含まれています。 `mediator_repair_scst`ツールは独自のツールであり、メインインストーラーとは別に、ONTAP Mediator `tgz`ファイル内にパッケージされています。ONTAP Mediator のインストールまたはアップグレード手順には影響しません。
