...
* 新しいバージョンのONTAP Mediatorにアップグレードすると、上位バージョンが利用可能でないかぎり、推奨されるSCSTバージョンに自動的にアップグレードされます。より新しいバージョンの SCST を手動でインストールする方法については、link:manage-task.html["ONTAP Mediatorの管理"]を参照してください。サポートされているバージョンについては、link:whats-new-concept.html#scst-support-matrix["SCSTのサポート対応表"]を参照してください。
