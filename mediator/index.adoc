...
[CAUTION]
====
* ONTAP Mediator が MetroCluster IP 構成で HTTPS プロトコルを使用して構成されている場合、単一の ONTAP Mediator インスタンスで監視できる MetroCluster IP 構成は 1 つだけです。複数の MetroCluster IP 設定を同時に監視することはサポートされていません。
