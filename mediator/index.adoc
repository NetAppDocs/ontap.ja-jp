...
* SCST パッケージを含む新しい ONTAP Mediator バージョンにアップグレードすると、より上位のバージョンが利用可能でない限り、インストーラーは推奨される SCST バージョンに自動的にアップグレードします。より新しいバージョンの SCST を手動でインストールする方法については、link:manage-task.html["ONTAP Mediatorの管理"]を参照してください。サポートされているバージョンについては、link:whats-new-concept.html#scst-support-matrix["SCSTのサポート対応表"]を参照してください。
...
* ONTAP Mediator では、SCSTパッケージはMetroCluster IP構成における通信にiSCSIを提供するために使用されます。SCSTパッケージは、インストール時にコンパイルされるカーネルモジュールです。ONTAP 9.19.1およびONTAP Mediator 1.12以降では、iSCSIの代わりにHTTPSを使用してMetroCluster IP構成を監視するONTAP Mediatorを設定できます。
...
* 22/tcpポートは通常の動作には必要ありませんが、メンテナンスのために一時的に有効にし、メンテナンスセッションが終了したら無効にすることができます。
...
NOTE: HTTPS を使用して通信を行う SnapMirror アクティブ同期または MetroCluster IP ONTAP Mediator の場合、ONTAP ではポート 3260 を有効にしたり接続したりする必要はありません。
...
NOTE: HTTPS のみをサポートする ONTAP Mediator 1.12 以降をインストールした場合、SCST パッケージはインストールされないため、カーネル互換性の要件は適用されません。
...
* お使いのONTAP Mediatorリリースでサポートされているバージョンを超えてカーネルをアップグレードしないでください。テスト済みのSCSTモジュールが動作しない可能性があります。
