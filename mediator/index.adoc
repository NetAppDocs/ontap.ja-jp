...
1.5
 a| 
. オペレーティングシステムを RHEL / CentOS のバージョンからアップグレードします。
. ホストをリブートしてカーネルモジュールの変更を適用します。ホストをリブートしないと、リブートを実行するように求めるエラーメッセージが表示されます。
. メディエーターのバージョンを今すぐ前のバージョンから現在のバージョンにアップグレードします。
...
+
Linux環境でLink Aggregation Control Protocol（LACP；リンクアグリゲーション制御プロトコル）を使用する場合は、カーネルを正しく設定し、を確認してください `sysctl net.ipv4.conf.all.arp_ignore` は「2」に設定されています。
...
. インストーラパッケージを解凍します。 `tar xvfz ontap-mediator-1.5.0.tgz`
+
[listing]
----
...
* `csc-prod-chain-ONTAP-Mediator.pem` |公開認証CAの信頼チェーン
* `csc-prod-ONTAP-Mediator.pem` |キーの生成に使用される証明書
* `ontap-mediator-1.5.0` |バージョン1.5.0の製品インストール実行可能ファイル
* `ontap-mediator-1.5.0.sig` | SHA-256のハッシュ化、CSC-prodキーを使用したRSA署名、インストーラの署名
* `ontap-mediator-1.5.0.sig.tsr` |インストーラの署名のためにOCSCPによって使用される取り消し要求
* `tsa-prod-ONTAP-Mediator.pem` | TSRの公開証明書
* `tsa-prod-chain-ONTAP-Mediator.pem` | TSR用のパブリック証明書CAチェーン


.手順
. 失効チェックをオンにします `csc-prod-ONTAP-Mediator.pem` Online Certificate Status Protocol（OCSP）を使用します。
+
.. 証明書を開発者証明書に登録するためのOCSP URLで、URIが指定されていない場合があることを確認します。
+
[listing]
----
...
----
.. OCSP Managerに接続してOCSP要求を送信します。 `openssl`
+
[listing]
----
...
. CSCの信頼チェーンと、ローカルホストに対する有効期限を確認します。 `openssl verify`
+
...
----
. を確認します `ontap-mediator-1.5.0.sig.tsr` および `ontap-mediator-1.5.0.tsr` 関連する証明書を使用しているファイル： `openssl ts -verify`
+

NOTE: `.tsr` ファイルには、インストーラとコード署名に関連付けられたタイムスタンプ応答が含まれます。タイムスタンプにTSAからの有効な署名があり、入力ファイルが変更されていないことが確認されます。検証はマシン上でローカルに実行されます。TSAサーバへのアクセスは不要です。
...
----
. キーに対して署名を確認します。  `openssl -dgst -verify`
+
[listing]
----
...
+
** READMEファイルの指示に従います。 `/opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/README.module-signing`SCSTカーネルモジュールに署名します。
** 必要なキーを見つけます。 `/opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys`
...
.手順
. インストーラを実行し、必要に応じてプロンプトに応答します。 `./ontap-mediator-1.5.0/ontap-mediator-1.5.0 -y`
+
[listing]
----
...
.ONTAP メディエーター1.5のインストールの例（コンソール出力）
[%collapsible]
====
[listing]
----
...
+
.. を実行します `systemctl status ontap_mediator`
+
[listing]
----
...
----
.. を実行します `systemctl status mediator-scst`
+
[listing]
----
...
. ONTAP メディエーターサービスで使用しているポートを確認します。 `netstat`
+
[listing]
----
