...
[CAUTION]
====
* MetroCluster IP構成において、ONTAP MediatorがHTTPSプロトコルで設定されている場合、単一のONTAP Mediatorインスタンスは1つのMetroCluster IP構成のみを監視できます。複数の MetroCluster IP 設定を同時に監視することはサポートされていません。
...
**  `No`ONTAP MediatorをSnapMirrorアクティブ同期にのみ使用する予定の場合、またはHTTPSを使用してONTAP MediatorインスタンスごとにONTAP Mediatorを設定して単一のMetroCluster IP構成を監視する予定の場合に応答してください。
+
または、 `./ontap-mediator-1.12.0/ontap-mediator-1.12.0 -y`コマンドの `--https-only`オプションを使用して、HTTPSのみを指定することもできます。
...
[NOTE]
====
インストール中に署名のチェックをスキップするには、次のコマンドを使用します： `./ontap-mediator-1.12.0/ontap-mediator-1.12.0 -y --skip-code-signature-check`
