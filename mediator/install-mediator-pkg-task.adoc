...
[CAUTION]
====
* MetroCluster IP構成において、ONTAP MediatorがHTTPSプロトコルで設定されている場合、単一のONTAP Mediatorインスタンスは1つのMetroCluster IP構成のみを監視できます。複数のMetroCluster IP構成を同時に監視することはサポートされていません。
