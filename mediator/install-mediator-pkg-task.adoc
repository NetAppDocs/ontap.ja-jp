...
summary: ONTAP Mediator をインストールするには、インストールパッケージを取得し、ホスト上でインストーラを実行する必要があります。 
---
...
NOTE: インストール中に署名のチェックをスキップするには、次のコマンドを使用します： `./ontap-mediator-1.11.0/ontap-mediator-1.11.0 -y --skip-code-signature-check`
...
.ONTAP Mediatorのインストール例（コンソール出力）
[%collapsible]
====
[listing]
----
