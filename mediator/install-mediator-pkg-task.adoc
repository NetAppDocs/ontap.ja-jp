...
summary: ONTAP Mediatorをインストールまたはアップグレードするには、インストールパッケージを入手し、ホスト上でインストーラーを実行する必要があります。 
---
= ONTAP Mediatorのインストール パッケージのインストール
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* ONTAP Mediator 1.12以降へのインストールまたはアップグレード中に、インストーラから次の応答を求められます：
+
...
+
** iSCSI で設定された ONTAP Mediator を使用して 1 つ以上の MetroCluster IP 構成を監視する予定がある場合は、応答してください `Yes`。
**  `No`に応答します。ONTAP Mediator を SnapMirror アクティブ同期にのみ使用する予定の場合、または HTTPS を使用して ONTAP Mediator を設定し、1 つ以上の MetroCluster IP 構成を監視する予定の場合に選択してください。
+
または、 `./ontap-mediator-1.12.1/ontap-mediator-1.12.1`コマンドの `--https-only`オプションを使用して、HTTPSのみを指定することもできます。



* 自動化のユースケースでは、 `./ontap-mediator-1.12.1/ontap-mediator-1.12.1 -y`コマンドを使用してインストーラーのプロンプトをスキップできます。
...
[NOTE]
====
インストール中に署名のチェックをスキップするには、次のコマンドを使用します： `./ontap-mediator-1.12.1/ontap-mediator-1.12.1 --skip-code-signature-check`

====
+
インストーラーは必要なアカウントを作成し、必要なパッケージをインストールします。Mediatorが既にインストールされている場合は、アップグレードを促すメッセージが表示されます。



.ONTAP Mediator のインストール例（HTTPS および iSCSI サポート付き）（コンソール出力）
[%collapsible]
====
[listing]
----
...
----
====
.ONTAP Mediator HTTPSおよびiSCSIサポートを含むアップグレードの例（コンソール出力）
[%collapsible]
====
[listing]
----
...
----
====
.ONTAP Mediator のインストール例（HTTPS のみをサポート）（コンソール出力）
[%collapsible]
====
[listing]
----
...
----
====
.HTTPSのみをサポートするONTAP Mediatorのアップグレードの例（コンソール出力）
[%collapsible]
====
[listing]
----
...
== UEFIセキュアブートのセキュリティキーを登録する

ONTAP Mediator 1.4以降、UEFIシステムでSecure Bootメカニズムが有効になっています。Secure Bootを有効にすると、インストール後にセキュリティキーを登録するための追加手順が必要になります。


NOTE: HTTPS のみをサポートする ONTAP Mediator 1.12 以降をインストールした場合、 SCST パッケージはインストールされていないため、このタスクはスキップできます。
...
NOTE: インストール後、システム出力に README ファイルとキーの場所が表示されます。
...
[NOTE]
====
秘密鍵はデフォルトの場所に置いておくことも、安全な場所に移動することもできます。公開鍵はBoot Managerが使用できるように、既存の場所に保管する必要があります。詳細については、README.module-signingファイルをご覧ください：

`[root@hostname ~]# ls /opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/
README.module-signing  scst_module_key.der  scst_module_key.priv`

====
. ホストを再起動し、デバイスのUEFI Boot Managerを使用して新しいMOKを承認します。link:index.html#install-ontap-mediator-when-uefi-secure-boot-is-enabled["UEFIセキュアブートが有効になっている場合はONTAP Mediatorをインストールします"]の `mokutil`ユーティリティで提供されたパスフレーズが必要になります。
...
ONTAP Mediator をインストールした後、systemctl status `mediator-scst`が failed（inactive）と表示される場合は、次の手順に従って SCST カーネル モジュールに署名します。


NOTE: HTTPS のみをサポートする ONTAP Mediator 1.12 以降をインストールした場合、 SCST パッケージはインストールされていないため、このタスクはスキップできます。

.手順
. ビルド プロセス中に、次のコマンドを使用して `/opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/`ディレクトリに公開キーと秘密キーのペアが生成されます：
+
`[root@mediator-host ~]# ls /opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/
README.module-signing  scst_module_key.der  scst_module_key.priv
[root@mediator-host ~]#`

. 次のコマンドを実行して、公開キーを UEFI キー リポジトリにインポートするプロセスを開始します：
+
...
. mokutil ソフトウェアは、インポート プロセス中にこのキーに使用する一時パスワードを要求します。
. インポートプロセスが `mokutil --list-new`で開始されたかどうかを確認してから、システムを再起動します。ブートローダーがEFI MOKマネージャーを起動します。
. 画面上のメニューを使用して、SCSTカーネルモジュールキーをオンにします。起動後、 `systemctl status mediator-scst`を実行します。サービスが起動すると、SCSTカーネルモジュールが署名されます。
