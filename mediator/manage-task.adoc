...
+
PKI機関はリクエストを検証し、署名します。 `.csr` 、証明書を生成する `intermediate.crt`。さらに、 `root_ca.crt`署名した証明書 `intermediate.crt`PKI 機関からの証明書。

+

NOTE: SnapMirror Business Continuity（SM-BC）クラスタの場合は、 `intermediate.crt`そして `root_ca.crt`ONTAPクラスタに証明書を追加します。見るlink:../snapmirror-active-sync/mediator-install-task.html["SnapMirror Active Sync 用に ONTAP Mediator とクラスタを構成する"]。
...
. 更新する `/opt/netapp/lib/ontap_mediator/uvicorn/config.json`ファイル。
+
値を更新する `ssl_keyfile`、 `ssl_certfile` 、 そして `ssl_ca_certs`:
...
+
** その `ssl_keyfile`値はキーパスです `ontap_mediator_server.crt`ファイル、これは `ontap_mediator_server.key`。
** その `ssl_certfile`値は、 `ontap_mediator_server_chain.crt`ファイル。
** その `ssl_ca_certs`値は、 `root_ca.crt`ファイル。
...
----
+
.. 証明書構造を使用している場合、 `root_ca.crt`証明書は `intermediate.crt`署名する証明書 `ontap_mediator_server.crt`証明書の場合は、 `intermediate.crt`そして `ontap_mediator_server.crt`証明書:
+
