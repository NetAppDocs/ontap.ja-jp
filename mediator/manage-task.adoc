...
NOTE: SnapMirror Business Continuity（SM-BC）クラスターと、HTTPS で構成された ONTAP Mediator を使用する MetroCluster IP 構成の場合は、 `intermediate.crt` および  `root_ca.crt` 証明書を ONTAP クラスターに追加する必要があります。link:../snapmirror-active-sync/mediator-install-task.html["SnapMirror アクティブ同期のために ONTAP Mediator とクラスタを設定する"] および link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/task_configuring_the_ontap_mediator_service_from_a_metrocluster_ip_configuration.html["MetroCluster IP構成用のONTAP Mediatorを構成します"^] を参照してください。
