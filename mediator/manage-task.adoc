...
NOTE: SnapMirrorビジネス継続性（SM-BC）クラスタの場合は、証明書と証明書をONTAPクラスタに追加する必要があります `ca.crt` `root_ca.crt` 。を参照して link:../snapmirror-active-sync/mediator-install-task.html["ONTAPメディエーターとクラスタのSnapMirrorアクティブ同期の設定"]
...
NOTE: SnapMirrorビジネス継続性（SM-BC）クラスタの場合は、証明書と証明書をONTAPクラスタに追加する必要があります `intermediate.crt` `root_ca.crt` 。を参照して link:../snapmirror-active-sync/mediator-install-task.html["ONTAPメディエーターとクラスタのSnapMirrorアクティブ同期の設定"]
