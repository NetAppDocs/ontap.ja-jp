...
NOTE: システムが以下の要件を満たしていない場合は、検証プロセスは必要なく、に直接移動できます。link:install-mediator-pkg-task.html["ONTAPメディエーターインストールパッケージのインストール"]
...
----
. 関連付けられた証明書を使用してファイルと `ontap-mediator-1.9.0.tsr`ファイルを確認し `ontap-mediator-1.9.0.sig.tsr`ます。
+
`openssl ts -verify`
