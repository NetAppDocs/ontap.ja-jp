...
[NOTE]
====
* 2025年6月15日以降、コード署名検証証明書の有効期限が切れているため、 ONTAP Mediator 1.8および1.9のインストールまたはアップグレードはできません。代わりに、 ONTAP Mediator 1.10をインストールまたはアップグレードしてください。
...
----
. 確認する `ontap-mediator-1.10.sig.tsr`そして `ontap-mediator-1.10.tsr`関連する証明書を使用するファイル:
+
`openssl ts -verify`
