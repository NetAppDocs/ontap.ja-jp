...
[NOTE]
====
* 2025年6月15日以降は、コード署名検証証明書の有効期限が切れているため、ONTAP Mediator 1.9および1.8をインストールまたはアップグレードできません。代わりに、ONTAP Mediator 1.10以降をインストールまたはアップグレードしてください。
...
----
. 関連する証明書を使用して、 `ontap-mediator-1.12.1.sig.tsr`および `ontap-mediator-1.12.1.tsr`ファイルを検証します：
+
[role="tabbed-block"]
====
.OpenSSL 3.x
