...
summary: ONTAP Mediator をインストールしたら、 ONTAP Mediator が実行中であることを確認する必要があります。 
---
= ONTAP Mediatorのインストールステータスを確認する
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: HTTPS のみをサポートする ONTAP Mediator 1.12 以降をインストールした場合、 SCST パッケージはインストールされていないため、この手順はスキップできます。
