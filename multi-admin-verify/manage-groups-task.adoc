...
MAVを設定して、承認リクエストが保留中であることをMAV管理者に通知できます。そのためには、Eメール通知、特にを設定する必要があります `Mail From` および `Mail Server` パラメータ--またはこれらのパラメータをクリアして通知を無効にすることができますMAV管理者は、電子メールアラートを使用しないで、承認キューを手動でチェックする必要があります。
...
** `-approvers` - 1人または複数の承認者のリスト
** `-email` -リクエストの作成、承認、拒否、または実行時に通知される1つ以上の電子メールアドレス。
+
