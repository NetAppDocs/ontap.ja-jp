...
MAVシステムデフォルトコマンドのルール– `security multi-admin-verify` コマンド–変更できません。

ルールを作成する際、必要に応じてを指定できます `-query` 要求をコマンド機能のサブセットに制限するオプション。たとえば、デフォルトのsetコマンドでは、 `-query` がに設定されます `-privilege diag`つまり、setコマンドに対して要求が生成されるのは、-privilege diagを指定した場合だけです。
...
----
デフォルトでは、ルールは対応するを指定します `security multi-admin-verify request create _“protected_operation”_` 保護された操作を入力すると、コマンドが自動的に生成されます。このデフォルト設定を変更して、を要求することができます `request create` コマンドは個別に入力してください。
...
NOTE: すべて `security multi-admin-verify rule` コマンドを実行するには、を除くMAV管理者の承認が必要です `security multi-admin-verify rule show`。
