...
** を設定して権限付きアクセスを許可する場合は `-allow-privileged-access` パラメータの値 `yes`また、権限付きアクセスのための特権ユーザ名も設定する必要があります。
** パススルーリードを設定するには、を設定します `-is-passthrough-read-enabled` パラメータの値 `true`また、権限付きデータアクセスも設定する必要があります。
+
次のコマンドは、 "event1" というイベントと、 "engine1" という外部エンジンが関連付けられた "policy1 `" という名前のポリシーを作成します。このポリシーではデフォルト値をポリシー設定に使用しています。
