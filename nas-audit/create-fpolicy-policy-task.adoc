...
+
** FPolicyポリシーには、カンマ区切りのイベントを1つ以上追加できます。
...
+
以下のコマンドは、 `policy1`という名前のポリシーを作成します。このポリシーには、 `event1`という名前のイベントと `engine1`という名前の外部エンジンが関連付けられています。このポリシーでは、ポリシー設定のデフォルト値を使用します：
`vserver fpolicy policy create -vserver vs1.example.com -policy-name policy1 -events event1 -engine engine1`

+
以下のコマンドは、 `policy2`という名前のポリシーを作成します。このポリシーには、 `event2`という名前のイベントと `engine2`という名前の外部エンジンが関連付けられています。このポリシーは、指定されたユーザー名を使用して特権アクセスを行うように構成されています。パススルー読み取りが有効になっています：
...
+
以下のコマンドは、 `native1`という名前のポリシーを作成します。このポリシーには `event3`という名前のイベントが関連付けられています。このポリシーはネイティブエンジンを使用し、ポリシー設定のデフォルト値を使用します。
