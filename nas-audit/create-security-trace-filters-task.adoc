...
.例
次のコマンドは、共有パスのファイルにアクセスするすべてのユーザを対象としたセキュリティトレースフィルタを作成します `\\server\share1\dir1\dir2\file.txt` IPアドレス10.10.10.7から取得します。フィルタは、の完全なパスを使用します `-path` オプションデータへのアクセスに使用されるクライアントの IP アドレスは 10.10.10.7 です。フィルタは 30 分後にタイムアウトします。
