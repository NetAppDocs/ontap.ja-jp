...
[role="lead"]
Storage Virtual Machine （ SVM ）に対して監査ポリシー変更イベントが設定されている場合、監査を有効にしたときに、それらについての監査イベントが生成されます。監査ポリシー変更イベントは、を使用して監査ポリシーが変更されたときに生成されます `vserver audit` 関連コマンド。
