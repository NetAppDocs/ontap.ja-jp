...
ユーザアカウントイベント。イベント ID は 4720 、 4722 、 4724 、 4725 、 4726 、 4738 および 4781 は、システムでローカル SMB または NFS ユーザが作成または削除され、ローカルユーザアカウントが有効化、無効化または変更され、ローカル SMB ユーザのパスワードがリセットまたは変更された場合に生成されます。ユーザアカウントイベントは、を使用してユーザアカウントが変更されたときに生成されます `vserver cifs users-and-groups _<local user>_` および `vserver services name-service _<unix user>_` コマンド
