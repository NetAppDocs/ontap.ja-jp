...
.例
次のコマンドでは、ワイルドカード文字を使用して、パスの下にあるすべてのファイルおよびディレクトリに関する情報を表示しています `/1/` SVM vs1のノード数：
    ...
    Expanded Dos Attributes: -
     ...
     Unix Mode Bits in Text: rwxrwxrwx
                       ACLs: NTFS Security Descriptor
                             ...
                             ALLOW-Everyone-0x1f01ff-OI|CI (Inherited)
----
次のコマンドでは、パスの下に「*」という名前のファイルの情報が表示されています `/vol1/a` SVM vs1に割り当てられます。パスは二重引用符（ "" ）で囲まれます。
