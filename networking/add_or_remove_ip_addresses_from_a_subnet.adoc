...
.このタスクについて
IP アドレスを追加するときに、追加しようとしている範囲の IP アドレスを使用しているサービスプロセッサまたはネットワークインターフェイスがあるとエラーが表示されます。手動でアドレスを指定したインターフェイスを現在のサブネットに関連付ける場合は、を設定します `-force-update-lif-associations` オプションをに設定します `true`。

IP アドレスを削除するときに、削除する IP アドレスを使用しているサービスプロセッサまたはネットワークインターフェイスがあるとエラーが表示されます。インターフェイスのIPアドレスをサブネットから削除したあとも、インターフェイスでそのアドレスを使用する場合は、を設定します `-force-update-lif-associations` オプションをに設定します `true`。
