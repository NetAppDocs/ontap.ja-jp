...
summary: すべてのSSL接続において、Federal Information Processing Standards 140-2に準拠しています。 
---
= FIPSを使用してONTAPネットワークセキュリティを設定する
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
|===
FIPSを有効にする前に、既存のSSH公開鍵アカウントをサポートされている鍵タイプを使用するように再設定する必要があります。そうしないと、管理者認証が失敗します。
...
. ONTAP 9.9.1以降では、再起動は不要です。ONTAP 9.8以前を実行している場合は、クラスタ内の各ノードを個別に手動で再起動してください。
...
以下のコマンドでSSLv3を有効にできます。ONTAP 9.9.1以降を実行している場合は、警告メッセージは表示されません。
...
.手順
. ONTAP 9.8以前を実行している場合は、クラスタ内の各ノードを個別に手動で再起動してください。
. 現在の準拠ステータスを表示します。
+
