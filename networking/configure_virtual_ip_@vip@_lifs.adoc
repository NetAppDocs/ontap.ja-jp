...
....
+
*** 「 asn_binteger 」には ASN を指定します。ONTAP 9.8 以降では、 BGP の ASN は 2 バイトの非負の整数をサポートします。これは 16 ビットの数値です（ 1 ~ 65534 を使用可能）。ONTAP 9.9..1 以降では、 BGP の ASN は 4 バイトの非負の整数（ 1 ～ 4294967295 ）をサポートしています。デフォルトの ASN は 65501 です。ASN 23456 は、 4 バイト ASN 機能を発表していないピアとの ONTAP セッション確立用に予約されています。
