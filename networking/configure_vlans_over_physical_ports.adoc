...
ONTAP 9.14.1 および 9.13.1 では、論理インターフェイス (LIF) によって使用されず、接続されたスイッチ上でネイティブ VLAN 接続がないタグなしポートは、劣化としてマークされます。これは、使用されていないポートを特定しやすくするためのもので、停止を示すものではありません。ネイティブVLANでは、ONTAP CFMブロードキャストなどのタグなしトラフィックをifgrpベース ポートで許可します。タグなしトラフィックをブロックしないように、スイッチのネイティブVLANを設定します。
