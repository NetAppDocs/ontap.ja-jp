...
* の値 `cdpd.holdtime` オプションの値はHAペアの両方のノードで同じに設定する必要があります。
...
* の値 `cdpd.interval` オプションの値はHAペアの両方のノードで同じに設定する必要があります。
...
.このタスクについて
ONTAP 9.10.1以前の場合は、 `lldp.enable` オプションは、ノードのポートでLLDPを有効または無効にするかを制御します。

* `on` すべてのポートでLLDPをイネーブルにします。
* `off` すべてのポートでLLDPをディセーブルにします。


ONTAP 9.11.1以降の場合は、 `lldp.enable` オプションは、ノードの非クラスタポートと非ストレージポートでLLDPを有効または無効にするかを制御します。

* `on` クラスタ以外のすべてのポートとストレージ以外のポートでLLDPを有効にします。
* `off` クラスタ以外のすべてのポートとストレージ以外のポートでLLDPを無効にします。
...
.このタスクについて
* TTLは計算値であり、送信間隔の積です (`lldp.xmit.interval`）と保持の乗数です (`lldp.xmit.hold`）と入力します。
