...
summary: ONTAP 9.8以降では、IPセキュリティを使用してネットワーク トラフィックを保護するオプションがあります。IPsecは、ONTAPで使用できるデータ転送中または通信中の暗号化オプションの1つです。本番環境で使用する前に、IPsecの設定を準備する必要があります。 
---
= ONTAPネットワークのIPセキュリティについて
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
ONTAP 9.16.1以降を使用している場合、暗号化や整合性チェックなどの計算負荷の高い操作を、ストレージ ノードに取り付けられたネットワークインターフェイスコントローラ（NIC）カードにオフロードするオプションがあります。IPsec暗号化処理をNICカードにオフロードした場合のネットワークスループットのパフォーマンスへの影響は、IPsec暗号化を有効にしない平文処理と比較して約5%以下です。これにより、IPsecで保護されたネットワーク トラフィックのパフォーマンスとスループットが大幅に向上します。
...
* X50131A（IOモジュール、2PT、100/200GbE）
* X50131B（IOモジュール、2PT、200/400GbE、B）
* X60132A（4p、10G/25G Ethernet Controller）


ONTAP 9.17.1 では、次のイーサネット カードのサポートが追加されました。

* X50135A（IOモジュール、2PT、100GbE、Encrypt）
* X60135A（2p、40G/100G Ethernet Controller）


X50131A、X50131B、およびX50135Aカードは、以下のプラットフォームでサポートされています：
