...
NOTE: クラスタ間 LIF と管理 LIF が同じサブネットに設定されていると、管理トラフィックが外部のファイアウォールによってブロックされ、 AutoSupport 接続と NTP 接続が失敗する可能性があります。を実行してシステムをリカバリできます `network interface modify -vserver _vserver name_ -lif _intercluster LIF_ -status-admin up|down` コマンドを使用してクラスタ間LIFを切り替えてください。ただし、この問題を回避するには、クラスタ間 LIF と管理 LIF を別々のサブネットに設定する必要があります。
