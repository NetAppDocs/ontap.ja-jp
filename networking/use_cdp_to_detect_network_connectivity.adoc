...
summary: データセンターでは、ネイバーディスカバリプロトコルを使用して、物理システムまたは仮想システムのペアとそのネットワークインターフェイス間のネットワーク接続状況を確認できます。ONTAPは2つのネイバーディスカバリプロトコルをサポートしています（Cisco Discovery ProtocolおよびLink Layer Discovery Protocol）。 
---
...
近隣デバイスに関する情報を表示するには、イーサネットスイッチとルーターでもCDPを有効にする必要があります。
...
NOTE: ノード上でLIFが変更された場合、受信側デバイス（例えばイーサネットスイッチ）でCDP情報が更新されないことがあります。このような問題が発生した場合は、ノードのネットワークインターフェースをダウン状態に設定してから、アップ状態に設定してください。
