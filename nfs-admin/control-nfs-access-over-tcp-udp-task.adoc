...
summary: -tcp パラメータと -udp パラメータをそれぞれ変更することで、TCP および UDP を介したストレージ仮想マシンへの NFS アクセスを有効または無効にできます。これにより、NFS クライアントが環境内で TCP または UDP 経由でデータにアクセスできるかどうかを制御できます。 
---
