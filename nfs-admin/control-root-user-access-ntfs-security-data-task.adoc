...
summary: ONTAPを設定して、NFSクライアントがNTFSセキュリティ形式のデータにアクセスできるようにし、NTFSクライアントがNFSセキュリティ形式のデータにアクセスできるようにすることができます。NFSデータストアでNTFSセキュリティ形式を使用する場合は、rootユーザによるアクセスの処理方法を決定し、それに応じてStorage Virtual Machineを設定する必要があります。 
---
