...
summary: ONTAP 9以降では、署名とシーリングを設定して、Active Directoryサーバへのクエリに対するLDAPセッションセキュリティを有効にできます。Storage Virtual MachineのNFSサーバセキュリティ設定を、LDAPサーバの設定に対応するように設定する必要があります。 
---
