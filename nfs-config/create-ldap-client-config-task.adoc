...
====
.手順
. に対する適切な設定値を決定するには、LDAP管理者に問い合わせてください `vserver services name-service ldap client create` コマンドを実行します
+
...
+
*** を使用します `-ad-domain` Active DirectoryドメインでLDAPサーバの検出を有効にするオプション。
+
を使用できます `-preferred-ad-servers` 複数の優先Active Directoryサーバをカンマで区切ってIPアドレスで指定できます。クライアントが作成されたら、を使用してこのリストを変更できます `vserver services name-service ldap client modify` コマンドを実行します

*** を使用します `-servers` カンマで区切ったIPアドレスでLDAPサーバ（ADまたはUNIX）を指定するオプション。
+
[NOTE]
====
。 `-servers` オプションはONTAP 9.2で廃止されました。ONTAP 9.2以降では、がサポートされます `-ldap-servers` フィールドがに置き換わります `-servers` フィールド。この新しいフィールドには、 LDAP サーバのホスト名または IP アドレスを指定できます。
...
+
に設定すると `*true*` LDAPサーバでサポートされている場合、LDAPクライアントはサーバへの暗号化されたTLS接続を使用します。デフォルト値はです `*false*`。このオプションを使用するには、 LDAP サーバの自己署名ルート CA 証明書をインストールする必要があります。

+
[NOTE]
====
SVMでSMBサーバがドメインに追加されていて、LDAPサーバがSMBサーバのホームドメインのドメインコントローラの1つである場合は、を変更できます `-session-security-for-ad-ldap` オプションを使用します `vserver cifs security modify` コマンドを実行します
...
*** `-referral-enabled` {`true`|`false`｝は、LDAPリファーラル追跡を有効にするかどうかを示します。
+
