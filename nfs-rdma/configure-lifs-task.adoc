...
NOTE: 各RDMAポートは最大127個のLIFを持つことができます。LIFを追加した場合、またはLIFがその制限を超えるポートにフェイルオーバーした場合、新しく追加されたLIFまたはフェイルオーバーしたLIFはRDMAトラフィックを処理できません。
...
**  `-rdma-protocols`パラメータはリストを受け取りますが、デフォルトでは空です。 `roce`が値として追加されると、LIF は RoCE オフロードをサポートするポートでのみ設定できるようになり、LIF の移行とフェイルオーバーの両方に影響します。
