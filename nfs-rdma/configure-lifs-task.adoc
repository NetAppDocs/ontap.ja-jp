...
** 。 `-rdma-protocols` パラメータは、デフォルトでは空であるリストを受け入れます。いつ `roce` 値として追加されたLIFは、RoCEオフロードをサポートするポート上でのみ設定でき、ボットLIFの移行やフェイルオーバーに影響します。
...
.CLI の使用
--
. LIFのステータスは、で確認できます `network interface show` コマンドを実行しますサービスポリシーに data-nfs ネットワークインターフェイスサービスを含める必要があります。。 `-rdma-protocols` リストにはを指定する必要があり `roce`。上記のいずれかの条件に該当しない場合は、 LIF を変更します。
