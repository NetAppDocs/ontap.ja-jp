...
[role="lead"]
ONTAP 9.16.1以降では、NVMe/TCP接続用にTLSセキュア チャネルを設定できます。System ManagerかONTAP CLIを使用して、TLSが有効である新しいNVMeサブシステムを追加したり、既存のNVMeサブシステムでTLSを有効にしたりできます。ONTAP はTLS ハードウェア オフロードをサポートしていません。
