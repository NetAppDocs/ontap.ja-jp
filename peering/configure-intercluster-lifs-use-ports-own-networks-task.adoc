...
+
次の例は、そのポートを示しています `e0e` および `e0f` デフォルトブロードキャストドメインから削除しておきます。
...
+
次の例は、ブロードキャストドメインを作成します `ipspace-IC1-bd` をクリックします `ipspace-IC1`：
...
+
次の例は、クラスタ間LIFを作成します `cluster01_icl01` および `cluster01_icl02` ブロードキャストドメインに属している必要があります `ipspace-IC1-bd`：
...
+
次の例は、クラスタ間LIFを示しています `cluster01_icl01` および `cluster01_icl02` 指定します `e0e` ポートは'e0fポートにフェイルオーバーします
