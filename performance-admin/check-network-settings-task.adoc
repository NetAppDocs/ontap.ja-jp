...
管理トラフィックを処理する e0M ポートを除き、ブロードキャストドメイン内のすべてのポートの MTU サイズが同じです。ポートがブロードキャストドメインの一部である場合は、を使用します `broadcast-domain modify` 変更したブロードキャストドメイン内のすべてのポートのMTUを変更するコマンド。
