...
| xref:../ontap/system-admin/forward-command-history-log-file-destination-task.html[LIFサアヒス] | を使用できます `management-log-forwarding` 監査ログをリモートsyslogサーバに転送するために使用するLIFを制御するサービス 
|===
