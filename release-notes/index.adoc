...
ONTAP 9.18.1 は、データ インフラストラクチャの近代化をサポートするために、セキュリティ、スケーラビリティ、ストレージ管理の分野で新機能と拡張機能を提供します。これらの機能強化により、拡張性が向上し、運用効率が向上し、将来に向けてデータを保護できるようになります。新機能と強化機能の完全なリストについては、xref:whats-new-9181.adoc[ONTAP 9.18.1の新機能] 。

* xref:../networking/configure-backend-encryption.html[バックエンドクラスタのHA暗号化]
+
バックエンド クラスター ネットワークの暗号化に mTLS を使用する機能。
...
+
IPv6 アドレスによるハードウェア オフロードによる IPSec のサポート。

* xref:../anti-ransomware/use-cases-restrictions-concept.html#supported-configurations[ARP/AIはFlexGroupボリュームをサポートします]
+
NetApp FlexGroupボリュームは ARP/AI をサポートし、高度な機械学習ベースのランサムウェア検出と保護を提供します。

* xref:../san-admin/nvme-copy-offload.html[NVMeのコピーオフロード]
+
コピー操作をストレージに直接オフロードすることで、ホスト CPU とネットワークの負荷を軽減し、大規模なデータ転送を高速化します。

* xref:../s3-snapshots/list-restore-s3-snapshot.html[ストレージ駆動型 S3 バケットスナップショット復元]
+
