...
ONTAP 9.12.1 以降、System Manager はNetAppコンソールと統合されています。コンソールを使用すると、管理者は使い慣れたシステム マネージャー ダッシュボードを維持しながら、単一のコントロール プレーンからハイブリッド マルチクラウド インフラストラクチャを管理できます。 System Manager にサインインする場合、管理者はNetAppコンソールの System Manager インターフェイスにアクセスするか、System Manager に直接アクセスするかを選択できます。詳細はこちらxref:../sysmgr-integration-console-concept.html[System Manager とNetAppコンソールの統合] 。
