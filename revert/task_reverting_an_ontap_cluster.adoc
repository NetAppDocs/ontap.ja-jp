...
. 両方のノードが元の状態に戻ったら、クラスターの HA を再度有効にします。
+
[source, cli]
----
...
NOTE: クラスターHAを再度有効にすると、 `storage failover auto-giveback`設定がtrueに戻る場合があります。
...
----
. 両方のノードで `storage failover auto-giveback`設定を確認します：
+
[source, cli]
----
...
----
+
環境に別の設定が必要な場合は、 `storage failover modify -node <nodename> -auto-giveback <true|false>`コマンドを使用して変更してください。
...
* link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/Why_auto-giveback_enabled_after_re-enabling_storage_failover_setting["NetApp ナレッジベース：ストレージフェイルオーバー設定を再度有効にした後、自動ギブバックが有効になる理由"^]
