...
*** アグリゲートとコンスティチュエントを新しいバケット用に指定しない場合、新しいバケットは既存の FlexGroup に追加されますが、 `retention-mode` 設定が既存のバケットの  `retention-mode` 設定と一致し、かつ FlexGroup タイプ (`compliance`、 `non-snaplock`、 `enterprise`）の場合に限ります。詳細については、xref:../flexgroup/index.html[FlexGroupボリューム管理] を参照してください。
+
