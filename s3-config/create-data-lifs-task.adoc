...
. IPv6アドレスをに割り当てる場合は、を参照してください `-address` オプション：
+
.. を使用します `network ndp prefix show` 各種インターフェイスで学習されたRAプレフィックスのリストを表示するコマンド。
+
...
.例
次のコマンドは、で割り当てられたS3データLIFを作成する方法を示しています `my-S3-policy` サービスポリシー：
