...
ONTAP 9.19.1以降では、条件を使用して、オブジェクトのアップロードに対して条件付き書き込みを、オブジェクトの削除に対して条件付き削除を強制することができます。条件付き書き込みと削除は、オブジェクトの上書きや意図しない削除を防ぐ手段を提供します。
...
----
ONTAP 9.19.1以降では、条件を使用して、オブジェクトのアップロードに対して条件付き書き込みを、オブジェクトの削除に対して条件付き削除を強制することができます。次の例では、bucket1に対する条件付き書き込みを強制するためのバケットポリシーステートメント条件を作成します。
