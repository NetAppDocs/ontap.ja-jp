...
.タスク概要
オブジェクト ストア サーバを作成すると、rootユーザがUID 0で作成されます。このrootユーザに対してアクセス キーやシークレット キーは生成されません。ONTAP管理者は `object-store-server user regenerate-keys`コマンドを実行して、このユーザーのアクセスキーとシークレットキーを設定する必要があります。
