...
元のアクセスキーの有効期限が切れた場合でもS3リソースへのアクセスを維持するには、ONTAP 9.19.1以降、各S3ユーザーに対してlink:regenerate-access-keys.html#generate-a-second-s3-access-key["2つ目のアクセスキーを生成する"]できます。
...
----
+
 `-access-key`および `-secret-key`パラメータでユーザーを作成する際に、アクセスキーとシークレットキーを手動で定義することもできます。例：
...
.System Manager
--
. * Storage > Storage VM * を選択します。ユーザを追加するストレージ VM を選択し、 * Settings * を選択してから、 S3 の下にある image:icon_pencil.gif["編集アイコン"] を選択します。
. ユーザーを追加するには、* Users > Add * を選択します。
. ユーザの名前を入力します。
...
. *保存*を選択します。ユーザが作成され、そのユーザのアクセス キーとシークレット キーが生成されます。
