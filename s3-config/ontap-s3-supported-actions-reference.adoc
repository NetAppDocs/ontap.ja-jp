...
| PutBucketLifecycleConfiguration | ONTAP 9.13.1 * 有効期限アクションのみがサポートされています 
...
ONTAP 9.19.1以降、ONTAP S3は、オブジェクトのアップロードに対する条件付き書き込みと、オブジェクトの削除に対する条件付き削除の適用をサポートしています。条件付き書き込みと削除は、オブジェクトの上書きや意図しない削除を防ぐ手段を提供します。条件付き操作は、TLS接続を介してAWS_V4署名方式を使用してリクエストが行われた場合にのみ機能します。以下の条件付きヘッダーとオブジェクト操作がサポートされています：
...
ONTAP S3 はこの追加の URI クエリパラメータをサポートしています：
`uploadId`
 a| 
ONTAP 9.8
...
| DeleteObjects ONTAP S3 はこの追加ヘッダーをサポートしています：* `x-amz-bypass-governance-retention` | ONTAP 9.11.1 
...
ONTAP S3は以下の追加のURIクエリパラメータをサポートしています。
...
ONTAP S3は以下の追加URIパラメータをサポートしています。
...
* ONTAP S3 は、 `x-amz-content-sha256:
STREAMING-AWS4-HMAC-SHA256-PAYLOAD`を使用したチャンクアップロードの署名リクエストのサポートを追加しました。
