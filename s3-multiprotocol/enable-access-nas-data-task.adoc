...
+
.. *ストレージ* > *ストレージ VM* をクリックし、NAS 対応のストレージ VM を選択して、*設定* をクリックし、次に *プロトコル* > *S3* の下で image:icon_gear.gif["アクションアイコン"] をクリックします。
...
. NASプロトコルが設定されているStorage VMでS3を有効にします。
+
.. *クラスター > ストレージ VM* をクリックし、NAS 対応のストレージ VM を選択して、*設定* をクリックし、次に *プロトコル > S3* の下にある image:icon_gear.gif["アクションアイコン"] をクリックします。
.. 証明書タイプを選択します。システムで生成された証明書と独自の証明書のどちらを選択した場合も、その証明書がクライアント アクセスで必要になります。
.. ネットワーク インターフェイスを入力します。
...
+
** 今後シークレット キーは表示されません。
** 証明書情報を再度確認する必要がある場合は、*[クラスター] > [ストレージ VM]* をクリックし、ストレージ VM を選択して、*[設定]* をクリックします。
