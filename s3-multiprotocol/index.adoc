...
NOTE: S3 NAS バケット内のマルチパートオブジェクトは、部分的ではなく全体のサイズで揃える必要があります。たとえば、パートは 4MB または 4GB または同様のサイズになります。パートでは、4.5MB や 4000.5MB などの部分的なサイズや MB 未満のサイズは使用できません。
...
* CopyObject は SnapLock ボリュームではサポートされていません
