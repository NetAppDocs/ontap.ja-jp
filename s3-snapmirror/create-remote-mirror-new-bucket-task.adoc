...
[Save] をクリックすると、ソース Storage VM に新しいバケットが作成され、デスティネーション VM にミラーされます。
...
. ソースクラスタとデスティネーションクラスタに CA サーバ証明書をインストールします。
+
.. ソースクラスタで、 _destination_cluster ：「 securitycertificate install -type server-ca -vserver _src_svm _cert-name_dest_server_certificate_` 」から証明書をインストールします
.. デスティネーションクラスタで、 _source クラスタから証明書をインストールします。「 securitycertificate install -type server-ca -vserver _dest _svm -cert-name_src_server_certificate_`
+
外部の CA ベンダーが署名した証明書を使用している場合は、ソースとデスティネーションの SVM に同じ証明書を入力します。
