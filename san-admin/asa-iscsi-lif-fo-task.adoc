...
* iSCSI LIFのフェイルオーバーと移行は、LIFのホームポートのブロードキャストドメイン内で実行されます。を使用してブロードキャストドメイン設定を確認します。 `network port broadcast-domain show` コマンドを実行します
+

IMPORTANT: iSCSI LIFを必要に応じて両方のノード間で移行およびフェイルオーバーできるように、iSCSI LIFに関連付けられたブロードキャストドメインにHAペアの両方のノードを表すネットワークポートが含まれていることを確認します。
