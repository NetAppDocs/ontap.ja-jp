...
.開始する前に
実稼働 LUN が存在し、アプリケーション サーバーの WWPN またはイニシエータ ノード名を含む igroup にマップされている必要があります。LUN もフォーマットされ、ホストからアクセスできる必要があります。

.手順
. ホスト ファイルシステム バッファの内容をディスクに保存します。
+
ホスト オペレーティング システムが提供するコマンドを使用できます。この手順を SAN バックアップ前処理スクリプトの一部にすることもできます。
