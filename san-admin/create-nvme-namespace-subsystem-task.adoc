...
[role="lead"]
ストレージをプロビジョニングすると、NVMe ホストが ONTAP ストレージシステムに対してデータの読み取りと書き込みを行えるようになります。NVMe プロトコルを使用するシステムでは、1 つ以上の NVMe ネームスペースとサブシステムを作成する必要があります。各ネームスペースを NVMe サブシステムにマッピングすることで、ホストからのデータアクセスが可能になります。

NVMeサブシステムを含むStorage Virtual Machine（SVM）にNVMe/FCとNVMe/TCPの両方のネットワークインターフェイスがある場合、NVMeサブシステムにマッピングされているネームスペースには、NVMe/FCとNVMe/TCPの両方のトランスポートプロトコルでアクセスできます。FCのNVMeサポートは、ONTAP 9.4以降で利用できます。TCPのNVMeサポートは、ONTAP 9.10.1以降で利用できます。ネームスペースへのプロトコル固有のアクセスが必要な場合は、プロトコルごとにlink:configure-svm-nvme-task.html["SVMを設定する"]する必要があります。
