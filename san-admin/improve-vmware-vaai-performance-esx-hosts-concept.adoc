...
summary: ONTAPは、ESXホストでESX 4.1以降が実行されている場合、特定のVMware vStorage APIs for Array Integration機能をサポートします。これらの機能は、ESXホストからストレージ システムへの処理のオフロードを支援し、ネットワーク スループットを向上させます。ESXホストは、適切な環境でこれらの機能を自動的に有効にします。 
---
