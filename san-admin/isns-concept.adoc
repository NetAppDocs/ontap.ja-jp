...
summary: Internet Storage Name Serviceは、TCP/IPストレージ ネットワーク上でのiSCSIデバイスの自動検出と管理を可能にするプロトコルです。iSNSサーバは、ネットワーク上のアクティブなiSCSIデバイスに関する情報（IPアドレス、iSCSIノード名IQN、ポータル グループなど）を保持します。 
---
