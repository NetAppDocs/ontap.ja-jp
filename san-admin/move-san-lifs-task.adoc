...
summary: ノードをオフラインにする必要がある場合は、SAN LIFを移動することで、WWPNなどの構成情報を保持し、スイッチファブリックのゾーン変更を回避できます。SAN LIFは移動前にオフラインにする必要があるため、ホストトラフィックはホストのマルチパスソフトウェアに依存して、LUNへの中断のないアクセスを提供する必要があります。SAN LIFはクラスタ内のどのノードにも移動できますが、ストレージ仮想マシン間でSAN LIFを移動することはできません。 
---
= SAN LIFの移動
:allow-uri-read: 
:icons: font
