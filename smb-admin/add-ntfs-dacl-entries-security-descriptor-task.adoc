...
summary: NTFSセキュリティ記述子にDACLアクセス制御エントリを追加することは、ファイルまたはフォルダにNTFS ACLを設定および適用する2番目の手順です。各エントリは、どのオブジェクトへのアクセスが許可されているか、または拒否されているかを識別し、ACEで定義されたファイルまたはフォルダに対して、そのオブジェクトが実行できる操作と実行できない操作を定義します。 
---
