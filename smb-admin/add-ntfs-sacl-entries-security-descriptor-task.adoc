...
summary: SACLアクセス制御エントリをNTFSセキュリティ記述子に追加することは、SVM内のファイルまたはフォルダに対するNTFS監査ポリシーを作成するための2番目の手順です。各項目は、監査対象となるユーザーまたはグループを識別します。SACLエントリは、アクセス試行の成功または失敗のどちらを監査対象とするかを定義します。 
---
