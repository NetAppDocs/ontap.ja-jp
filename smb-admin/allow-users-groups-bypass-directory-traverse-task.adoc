...
summary: ユーザーが、通過するディレクトリに対する権限を持っていない場合でも、ファイルへのパス内のすべてのディレクトリを通過できるようにする場合は、Storage Virtual Machine上のローカルSMBユーザーまたはグループにSeChangeNotifyPrivilege権限を追加できます。デフォルトでは、ユーザーはディレクトリ通過チェックをバイパスできます。 
---
