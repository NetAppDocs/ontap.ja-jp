...
summary: 従来のオプロックとリースオプロックにより、特定のファイル共有シナリオでSMBクライアントはクライアント側で先読み、後書き、およびロック情報のキャッシュを実行できます。これにより、クライアントは、該当ファイルへのアクセスが必要であることをサーバに定期的に通知することなく、ファイルの読み書きを行うことができます。これにより、ネットワーク トラフィックが削減され、パフォーマンスが向上します。 
---
