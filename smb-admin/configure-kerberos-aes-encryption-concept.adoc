...
summary: Kerberosベースの通信におけるセキュリティを最大限に高めるには、SMBサーバでAES-256およびAES-128暗号化を有効にすることができます。デフォルトでは、SVM上にSMBサーバを作成すると、Advanced Encryption Standard（AES）暗号化は無効になります。AES暗号化による強力なセキュリティを活用するには、この機能を有効にする必要があります。 
---
