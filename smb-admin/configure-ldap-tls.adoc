...
CIFSサーバーが属するドメインには、Active Directory証明書サービスが既にインストールされ、構成されている必要があります。Active Directory証明書サービスのインストールと構成に関する情報は、 http://technet.microsoft.com/en-us/library/["Microsoft TechNetライブラリ：technet.microsoft.com/ja-jp/library/"]を参照してください。

[NOTE]
====
ONTAP 9.13.1 以降、 `-use-start-tls-for-ad-ldap`または `-use-ldaps-for-ad-ldap`が SMB サーバーで有効になっている場合は、信頼済みドメインとホームドメインの両方に必要な CA 証明書がインストールされていることを確認してください。ONTAP は、信頼済みドメインのドメイン コントローラに対して TLS または LDAPS 接続を確立し、そのサイト情報を取得できる必要があります。
...
[NOTE]
====
ONTAP 9.13.1 以降、Active Directory 接続でセキュアな LDAP 通信が有効になっている場合、ONTAP は信頼済みドメインのドメイン コントローラに接続する際にも、これらのセキュアな接続を使用します。信頼済みドメインのドメイン コントローラに必要な証明書がインストールされていない場合、ONTAP は信頼済みドメインのサイト情報を特定できない可能性があります。
