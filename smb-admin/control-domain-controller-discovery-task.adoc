...
summary: ONTAP 9.3以降では、ドメインコントローラが検出されるデフォルトのプロセスを変更できます。これにより、検出をサイトまたは優先DCのプールに制限でき、環境によってはパフォーマンスの向上につながる可能性があります。 
---
...
+
 `default-site`パラメータをSMBサーバーに定義することで、サイトとサービスでサイトに割り当てられていないLIFでもこのモードを使用できるようになります。ONTAP 9.13.1以降、ONTAPが信頼できるドメインのサイト情報を検出できない場合に、デフォルトサイトをフォールバックとして使用することもできます。
