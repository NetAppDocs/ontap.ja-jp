...
[NOTE]
====
および `-replacement`ステートメントは、 `-pattern`正規表現として記述できます。また、ステートメントを使用して、nullの置換文字列（スペース文字）を使用してユーザへのマッピングを明示的に拒否する `" "`こともできます `-replacement`。詳細については、のマニュアルページを参照して `vserver name-mapping create` ください。
