...
summary: ストレージ仮想マシン用の監査ポリシーを作成することは、ファイルまたはフォルダにACLを設定および適用する手順の3番目のステップです。ポリシーは様々なタスクを格納するコンテナとして機能し、各タスクはファイルやフォルダに適用できる単一のエントリです。セキュリティ ポリシーには後からタスクを追加できます。 
---
