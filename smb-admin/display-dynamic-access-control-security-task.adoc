...
summary: NTFSボリュームおよび、セキュリティ形式が混在するボリューム上のNTFS有効セキュリティが適用されるデータについて、Dynamic Access Controlセキュリティに関する情報を表示できます。これには、条件付きACE、リソースACE、および中央アクセス ポリシーACEに関する情報が含まれます。これらの結果は、セキュリティ設定の検証やファイル アクセスに関する問題のトラブルシューティングに利用できます。 
---
