...
summary: ONTAP CLIを使用して、FlexVolボリュームのNFSv4監査ポリシーに関する情報を表示できます。この情報には、セキュリティ形式と有効なセキュリティ形式、適用される権限、およびシステム アクセス制御リストに関する情報が含まれます。これらの結果は、セキュリティ設定の検証や監査に関する問題のトラブルシューティングに利用できます。 
---
