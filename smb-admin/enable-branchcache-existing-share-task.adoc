...
* を追加したとき `branchcache` 共有への共有プロパティ、既存の共有設定、および共有プロパティは維持されます。
+
BranchCache 共有プロパティは既存の共有プロパティリストに追加されます。を使用する方法の詳細については、を参照してください `vserver cifs share properties add` コマンドのマニュアルページを参照してください。
...
.例
次のコマンドでは、パス「data2」という名前の既存のSMB共有でBranchCacheを有効にします `/data2` SVM vs1の場合
