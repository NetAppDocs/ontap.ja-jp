...
summary: Kerberosベースの通信で最高レベルのセキュリティを活用するには、SMBサーバでAES-256およびAES-128暗号化を有効にすることができます。SMBサーバでActive Directory KDCとのKerberosベースの通信にAES暗号化タイプを選択しないようにする場合は、AES暗号化を無効にすることができます。デフォルトでは、AES暗号化は無効になっています。 
---
...
ONTAP 9.12.1以降では、 `-advertised-enc-types`オプションを使用してAES暗号化を有効化または無効化できます。このオプションでは、AD KDCにアドバタイズされる暗号化タイプを指定できます。デフォルト設定は `rc4`と `des`ですが、AESタイプを指定するとAES暗号化が有効になります。また、オプションを使用して、より弱いRC4およびDES暗号化タイプを明示的に無効にすることもできます。ONTAP 9.11.1以前では、 `-is-aes-encryption-enabled`オプションを使用してAES暗号化を有効化または無効化する必要があり、暗号化タイプを指定することはできません。
 ...
 `-advertised-enc-types`パラメータを変更すると、指定された暗号化タイプが以前に設定されたタイプを上書きします。以前の暗号化タイプの設定はすべて削除されます。

セキュリティを強化するため、ストレージ仮想マシン（SVM）は、AESセキュリティオプションが変更されるたびに、AD内のマシンアカウントのパスワードを変更します。パスワードを変更するには、マシンアカウントが含まれる組織単位（OU）の管理者権限を持つActive Directoryの認証情報が必要になる場合があります。資格情報の入力を求めるプロンプトは、ONTAPがActive Directoryでマシンアカウントのパスワードをリセットする必要があり、かつADの構成でマシンアカウントを含む組織単位の管理者資格情報が必要な場合にのみ表示されます。Active Directoryが同期されており、自動リセットが有効になっている場合は、プロンプトは表示されません。
