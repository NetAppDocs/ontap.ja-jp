...
summary: Oplockを使用すると、クライアントはファイルをロックしてコンテンツをローカルにキャッシュできるため、ファイル操作のパフォーマンスを向上させることができます。ストレージ仮想マシン上に存在するSMB共有では、Oplockが有効になっています。状況によっては、Oplockを無効にしたい場合もあるでしょう。Oplockは、共有ごとに有効または無効にすることができます。 
---
