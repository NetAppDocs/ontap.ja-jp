...
[NOTE]
====
共有にデフォルトの共有プロパティだけを設定する場合は、です `oplocks`、 `browsable`および `changenotify` enabledの場合、を指定する必要はありません `-share-properties` SMB共有の作成時に指定するパラメータ。デフォルト以外の共有プロパティを組み合わせて使用する場合は、を指定する必要があります `-share-properties` パラメータを指定し、その共有に使用する共有プロパティをリストします。
