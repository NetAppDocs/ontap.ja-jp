...
summary: Active Directory通信を保護するためにSSL/TLS経由のLDAPを使用するには、まずActive Directory証明書サービスの自己署名ルートCA証明書のコピーを証明書ファイルにエクスポートし、それをASCIIテキストファイルに変換する必要があります。このテキストファイルは、ONTAPが証明書をStorage Virtual Machineにインストールする際に使用されます。 
---
