...
summary: ID破棄構成でストレージ仮想マシンのディザスタリカバリ先に対してファイルディレクトリポリシーを適用する前に、留意すべき特定のガイドラインがあります。これは、ファイルディレクトリポリシー構成で、セキュリティ記述子またはDACLもしくはSACLエントリのいずれかにローカルユーザまたはグループを使用している場合です。 
---
