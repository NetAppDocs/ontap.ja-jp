...
summary: Auto Locationは、SMBの自動ノード参照機能を利用して、Storage Virtual Machine上のSMBクライアントのパフォーマンスを向上させます。自動ノード参照機能は、要求元のクライアントを、データが格納されているボリュームをホストしているノードSVM上のLIFに自動的にリダイレクトするため、クライアントの応答時間の改善につながります。 
---
