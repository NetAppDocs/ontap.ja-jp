...
| Windows 管理者 | `vserver cifs home-directory modify -vserver _vserver_name_ -is-home-dirs-access-for-admin-enabled {true{vbar}false}`
デフォルトはです `true`。 
...
.. アクセスを有効または無効にします。 `vserver cifs home-directory modify -vserver _vserver_name_ -is-home-dirs-access-for-public-enabled {true|false}` [+]
デフォルトはです `false`。
.. admin権限レベルに戻ります。+
`set -privilege admin`
