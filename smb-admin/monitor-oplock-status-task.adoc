...
を参照してください `vserver oplocks show` 各パラメータの詳細な概要 に関するマニュアルページ。
...
.例
次のコマンドは、すべてのロックに関するデフォルトの情報を表示します。表示されるファイルのoplockは、で許可されます `read-batch` oplockレベル：
...
----
次の例は、パスのファイルに対するロックに関する詳細情報を表示します `/data2/data2_2/intro.pptx`。ファイルでoplockリースが許可されています `batch` IPアドレスがのクライアントのoplockレベル `10.3.1.3`：
