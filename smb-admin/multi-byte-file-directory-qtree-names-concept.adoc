...
* ボリュームを設定します `-language` オプションを明示的に指定： `volume create -language utf8mb4 {…}`
* ボリュームを継承する `-language` オプションを指定して作成または変更されたSVMのオプション。 `vserver [create|modify] -language utf8mb4 {…}``volume create {…}`
