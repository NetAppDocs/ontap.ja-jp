...
summary: ストレージ仮想マシン上にローカルユーザとローカルグループを作成する理由はいくつかあります。たとえば、ドメインコントローラが使用できない場合はローカルユーザアカウントを使用してSMBサーバにアクセスできます。また、ローカルグループを使用して権限を割り当てたい場合や、SMBサーバがワークグループ内にある場合などがあります。 
---
