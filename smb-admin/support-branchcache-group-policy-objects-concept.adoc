...
+
BranchCacheのハッシュの発行GPOは、に対応します `-operating-mode` パラメータGPO の更新が行われると、グループポリシーが適用される組織単位（ OU ）に含まれる Storage Virtual Machine （ SVM ）オブジェクトにこの値が適用されます。

* * BranchCache のハッシュバージョンサポート *
+
BranchCacheのハッシュバージョンサポートGPOは、に対応します `-versions` パラメータGPO の更新が行われると、グループポリシーが適用される組織単位に含まれる SVM オブジェクトにこの値が適用されます。
