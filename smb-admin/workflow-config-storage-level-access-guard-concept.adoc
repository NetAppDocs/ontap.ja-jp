...
summary: Storage-Level Access Guardを設定するワークフローでは、NTFSファイル権限と監査ポリシーを設定する際に使用するものと同じONTAP CLIコマンドを使用します。指定されたターゲット上でファイルおよびディレクトリへのアクセスを設定する代わりに、指定されたStorage Virtual Machineボリューム上でSLAGを設定します。 
---
...
.関連情報
xref:configure-storage-level-access-guard-task.adoc[サーバーでStorage-Level Access Guardを設定する]
