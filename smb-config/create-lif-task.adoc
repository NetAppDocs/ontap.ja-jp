...
. LIF を作成します。
+
「 network interface create -vserver vserver_name -lif_lif_name_-role data -data-protocol cifs -home-node _node_name _home-port_name_{-address_IP_address_-netmask IP_address | -subnet-name _subnet_name_} -firewall-policy data -auto-revert ｛ true ｝
...
| * ONTAP 9.5 以前 * 
 ...
 a| 
「 network interface create -vserver vserver_name -lif_lif_name_-role data -data-protocol cifs -home-node _node_name _home-port_name_{-address_ip_address _netmask_ip_address _|-subnet-name _subnet_name _} -firewall-policy data-firmware {\false} 」を指定します
...
| * ONTAP 9.6 以降 * 
 ...
 a| 
「 network interface create -vserver _vserver_name -lif_lif_name_service-policy_policy_policy_service_policy_name_-home-port_node_name _-home-node _port_name_{-address_ip_address _| -subnet-name _subnet_name _} -firewall-policy data -false|} 」を指定します
