...
SMB サーバーを作成するには、組織単位（OU）に対して次の最低限の権限が必要です：

* コンピュータオブジェクトを作成する
* コンピュータオブジェクトを削除する
* パスワードをリセットする
* アカウントの読み取りおよび書き込みの制限
* DNSホスト名への検証済み書き込み
* サービスプリンシパル名への検証済み書き込み
* msDS-SupportedEncryptedTypesを読む
* msDS-SupportedEncryptedTypesを書き込む
...
*  `-ou`オプションを使用して、SMBサーバーを別のOUに追加することもできます。
* ONTAP 9.13.1 以降では、SMB サーバーのデフォルトサイトを設定できます。ONTAP は、LIF がサイトメンバーシップを持っていない場合、および信頼済みドメインのドメイン コントローラを検索する際に信頼済みドメインのサイト検出が失敗した場合のフォールバックとして、このサイトを使用できます。
...
. AD ドメインに SMB サーバを作成します： `+vserver cifs create -vserver vserver_name -cifs-server smb_server_name -domain FQDN [-ou organizational_unit][-default-site site_name][-netbios-aliases NetBIOS_name, ...][-keytab-uri {(ftp|http)://hostname|IP_address}][-comment text]+`
+
...
ONTAP 9.13.1 以降、ONTAP が信頼できるドメインのサイト情報を検出できない場合、SMB サーバーのデフォルト サイトもフォールバックとして使用されます。
