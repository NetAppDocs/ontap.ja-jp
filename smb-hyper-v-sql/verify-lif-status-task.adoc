...
summary: Hyper-VとSMB経由のSQL Serverを使用してStorage Virtual Machineを構成し、クラスタ内の各ノードにLIFを配置した場合でも、日常的な運用中に、一部のLIFが別のノードのポートに移動する可能性があります。LIFのステータスを確認し、必要な是正措置を講じる必要があります。 
---
= LIFステータスの確認
:allow-uri-read: 
:icons: font
