...
NOTE: `conf` がに追加されます `svl_fops_name` の場合 `scsi-vhci-failover-override` が適用されました。追加情報およびデフォルト設定への推奨される変更については、ネットアップの技術情報アーティクルを参照してください https://["Solaris ホストでは、 SnapMirror Business Continuity （ SM-BC ）構成での推奨設定がサポートされます"]。
