...
ONTAP 9.13.1以降では、ロックされたSnapshotコピーをSnapLockバックアップ関係のデスティネーションSnapLockボリュームに瞬時にリストアできます。これには、 `snaplock-type` オプションを「non-snaplock」に設定し、ボリュームクローン作成処理の実行時に「parent-snapshot」としてSnapshotコピーを指定します。の詳細を確認してください link:https://docs.netapp.com/us-en/ontap/volumes/create-flexclone-task.html?q=volume+clone["SnapLock タイプのFlexCloneボリュームを作成します"]。
...
.手順
. デスティネーションクラスタを特定します。
. デスティネーションクラスタで、SnapLockライセンスをインストールしてコンプライアンスクロックを初期化し、9.10.1より前のONTAPリリースを使用している場合は、の説明に従ってSnapLockアグリゲートを作成します。 xref:workflow-concept.html[SnapLock ワークフロー]。
