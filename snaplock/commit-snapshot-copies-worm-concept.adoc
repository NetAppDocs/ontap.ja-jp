...
+
この例では、デスティネーションSnapLock ボリュームとの新しいSnapMirror関係を作成します `dstvolB` ポリシーを使用する `XDPDefault` dailyおよびweeklyというラベルの付いたSnapshotコピーを毎時スケジュールで保管するには、次の手順を実行します。
...
----
. 関係が初期化されてアイドル状態になったら、を使用します `snapshot show` レプリケートされたSnapshotコピーに適用されたSnapLock の有効期限を確認するコマンド。
+
この例は、ボリュームのSnapshotコピーを表示します `dstvolB` SnapMirrorラベルとSnapLock 有効期限が設定されているデータ：
