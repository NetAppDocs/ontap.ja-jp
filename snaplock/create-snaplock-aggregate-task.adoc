...
summary: 'volume `-snaplock-type`オプションを使用して、ComplianceまたはEnterprise SnapLockボリュームタイプを指定します。ONTAP 9.10.1より前のリリースでは、個別のSnapLockアグリゲートを作成する必要があります。ONTAP 9.10.1以降では、SnapLockボリュームと非SnapLockボリュームを同じアグリゲート上に配置できます。したがって、ONTAP 9.10.1を使用している場合は、個別のSnapLockアグリゲートを作成する必要はありません。' 
---
