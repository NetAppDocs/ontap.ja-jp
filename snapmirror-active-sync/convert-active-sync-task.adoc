...
---
= 既存のONTAP SnapMirror関係をSnapMirror Active Sync 関係に変換する
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
.開始する前に
* プライマリ クラスタとセカンダリ クラスタの間には、ゼロ RPO SnapMirror同期関係が存在する必要があります。
...
* link:mediator-install-task.html["ONTAP Mediator"] SnapMirrorアクティブ同期を設定する必要があります。
