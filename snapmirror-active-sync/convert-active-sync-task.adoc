...
---
= 既存の ONTAP SnapMirror SAN 関係を SAN 関係の SnapMirror アクティブ同期に変換する
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
SnapMirror 保護を設定済みの場合は、SAN 関係を SnapMirror アクティブ同期（SAN 向け）に変換できます。ONTAP 9.15.1 以降では、関係を対称アクティブ / アクティブ保護を使用するように変換できます。

SVM-DR 関係を NAS 関係の SnapMirror アクティブ同期に変換する方法については、link:protect-nas-task.html#convert-an-svm-dr-relationship-to-a-snapmirror-active-sync-relationship["SVM DR関係をSnapMirror active sync関係に変換する"]を参照してください。
