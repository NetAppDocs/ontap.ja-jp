...
ONTAP 9.19.1以降、AIXの透過的アプリケーションフェイルオーバーは、対称アクティブ/アクティブ構成のSnapMirror active syncでサポートされています。System ManagerまたはONTAP CLIを使用してAIX LUNを作成できます。透過的なフェイルオーバーを保証するには、ONTAP CLIを使用する場合、lun createコマンドまたはlun modifyコマンドでオプションaccess-mode localを使用する必要があります。デフォルトでは、アクセスモードはglobalに設定されています。System Managerを使用する場合は、ホストとしてAIXを選択し、アクセスモードとしてlocalを選択します。LUNの作成の詳細については、link:../san-admin/provision-storage.html["SANストレージのプロビジョニング"]を参照してください。
