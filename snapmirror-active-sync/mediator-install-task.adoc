...
ONTAP 9.19.1 および ONTAP Mediator 1.12 以降では、同じ ONTAP Mediator インスタンスを使用して、4 ノードの MetroCluster IP 構成と SnapMirror アクティブ シンクのクラスタ ピア関係を同時に監視できます。構成は次の要件を満たしている必要があります。

* ONTAP Mediator 1.12.1以降が、2コアCPUを搭載したホストにインストールされています。
* ONTAP MediatorインスタンスはHTTPSプロトコルで構成されています。
+

NOTE: 同じ ONTAP Mediator インスタンスを使用して監視できるのは、4 ノードの MetroCluster IP 構成 1 つと、SnapMirror アクティブ同期ピア関係 1 つのみです。
...
ONTAP Mediatorを使用してSnapMirrorアクティブ同期関係を監視する場合、有効な自己署名証明書または認証局（CA）証明書がないと、ONTAP MediatorをONTAPで初期化できません。ピアクラスタの証明書ストアに有効な証明書を追加します。
...
-----END CERTIFICATE-----
----
. プライマリクラスタまたはセカンダリクラスタにルートCA証明書と中間CA証明書をインストールします。
+
.. ルートCA証明書をインストールします：
+
...
+
プロンプトが表示されたら、次の例に示すように `root.crt`を入力します。
...
----
. 前の手順を繰り返して、ルートCA証明書と中間CA証明書をピアクラスタにインストールします。
. 生成された証明書名を使用してインストールされた自己署名CA証明書を表示します：
+
