...
---
= ONTAP SnapMirror アクティブ同期で SAN を保護する
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
SnapMirror active syncを使用してSAN構成の非対称保護を構成するには、ONTAPソース クラスタ上のLUNを選択し、それらをコンシステンシー グループに追加します。
...
* ソースとデスティネーション クラスタは、MetroCluster 構成の一部にすることはできません。
...
System Manager または ONTAP CLI を使用して、SAN 構成の対称保護を確立できます。どちらのインターフェースにも、xref:index.html#key-concepts[均一な構成と非均一な構成] の手順が異なります。
...
--
.均一な構成の手順
. 主要サイトでは、link:../consistency-groups/configure-task.html#create-a-consistency-group-with-new-luns-or-volumes["新しいLUNを使用して整合性グループを作成する"]。
+
...
.System Manager
--
. 主要サイトでは、link:../consistency-groups/configure-task.html#create-a-consistency-group-with-new-luns-or-volumes["新しいボリュームまたはNVMeネームスペースを使用して整合性グループを作成する"]。
