...
---
= 設定済みONTAPテンプレートを使用したSVMのプロビジョニング
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.タスク概要
SVMの設定に使用できる設定済みテンプレートは、VserverEnvironmentSetupです。VserverEnvironmentSetupテンプレートを使用して次の設定を行うことができます。
...
VserverEnvironmentSetupテンプレートは読み取り専用です。このテンプレートのデフォルトのパラメータ値は変更できません。テンプレート内のパラメータのデフォルト値を変更するには、テンプレートをコピーしてから値を変更する必要があります。

.手順
. SVM を作成および設定するためのテンプレートで定義されているパラメータの値を表示してメモします： `template parameter show`
+
[listing]
----
...
----
+
新しいテンプレートは読み取り / 書き込みが可能です。

.. 目的のパラメータのデフォルト値を変更します。
+
...
. テンプレート内のパラメータに値を指定してSVMをプロビジョニングします。
+
