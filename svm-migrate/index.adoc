...
NOTE: AFFクラスターからハイブリッド アグリゲートを持つFASクラスターに移行する場合、自動ボリューム配置では、類似のアグリゲートの一致を実行しようとします。たとえば、ソース クラスタにボリュームが60個ある場合、ボリュームの配置では、デスティネーションにあってそれらのボリュームを配置できるAFFアグリゲートが検索されます。AFFアグリゲートに十分なスペースがない場合、ボリュームはフラッシュ ディスク以外のアグリゲートに配置されます。
