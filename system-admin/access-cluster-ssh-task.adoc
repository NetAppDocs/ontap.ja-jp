...
* ONTAP 9.19.1 以降、管理者はlink:https://docs.netapp.com/us-en/ontap-cli/security-ssh-publickey-show.html["security ssh publickey show -vserver  -host-key-algorithm  コマンド"^]を使用して ONTAP ホストキーのフィンガープリントを表示できます。これにより、管理者はこのフィンガープリントが ONTAP に保存されているフィンガープリントと一致することを確認できます。
