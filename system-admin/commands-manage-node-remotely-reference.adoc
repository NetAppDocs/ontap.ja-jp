...
SPおよびBMCで使用可能な `help`コマンドオプションの詳細については、link:online-help-at-sp-bmc-cli-task.html["ONTAP SP/BMC CLIのヘルプ情報を表示します"]を参照してください。

対応する ONTAP BMC CLI の詳細については、link:bmc-cli-commands-reference.html["ONTAPベースボード管理コントローラでサポートされているCLIコマンド"]を参照してください。
