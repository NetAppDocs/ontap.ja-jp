...
クラスタシェルでサポートされているノードシェルオプションには、 `vserver options clustershell` 指示。これらのオプションを確認するには、clustershell CLIで次のようにクエリを実行します。 `vserver options -vserver _nodename_or_clustername_ -option-name ?`
