...
summary: SSLサーバがクラスタまたはStorage Virtual MachineをSSLクライアントとして認証できるようにするには、クライアント タイプのデジタル証明書をクラスタまたはSVMにインストールします。次に、client-ca証明書をSSLサーバ管理者に提供して、サーバにインストールします。 
---
