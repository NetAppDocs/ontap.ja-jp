...
summary: クラスターと、Key Management Interoperability Protocolサーバーなどの外部キー マネージャとの間で相互認証を行うことで、キー マネージャはSSL上でKMIPを使用してクラスターと通信できるようになります。これは、アプリケーションや特定の機能が、安全なデータ アクセスを提供するためにセキュア キーを必要とする場合に行います。 
---
