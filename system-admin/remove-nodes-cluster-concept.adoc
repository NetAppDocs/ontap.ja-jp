...
.開始する前に
クラスターからノードを削除する前に、次の条件を満たしている必要があります。
...
NOTE: HA ペアの最初のノードでこの手順を実行する場合は、そのノードのみを不適格としてマークする必要があります。 HA パートナーのステータスを変更しないでください。パートナー ノードが新しいマスターとして選択された場合は、そのノードが不適格となる前にイプシロンを保持しているかどうかを確認する必要があります。これは、これらの手順を繰り返してパートナー ノードを削除すると実行されます。
