...
summary: Storage Virtual Machine を対象とした NDMP モードでは、NDMP ユーザ認証がロールベースのアクセス制御と統合されています。SVM コンテキストでは、NDMP ユーザは vsadmin または vsadmin-backup のいずれかのロールを持っている必要があります。クラスタ コンテキストでは、NDMP ユーザは admin または backup のいずれかのロールを持っている必要があります。 
---
