...
** `*volumeAdd.yaml*` （または `*volumeEdit.yaml*`）：から入力を読み取るときに値を作成または変更するために必要なテストケース `variable.yaml` ファイル。
...
** `*lunAdd.yaml*` （または `*lunEdit.yaml*`）：から入力を読み取るときに値を作成または変更するために必要なテストケース `variable.yaml` ファイル。
