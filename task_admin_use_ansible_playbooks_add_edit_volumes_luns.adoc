...
summary: ONTAP 9.9.9以降では、ボリュームまたはLUNを追加または編集する場合に、System ManagerでAnsibleプレイブックを使用できます。 
---
...
[role="lead"]
ONTAP 9.9.9以降では、ボリュームまたはLUNを追加または編集する場合に、System ManagerでAnsibleプレイブックを使用できます。
...
** `*volumeAdd.yaml*` （または `*volumeEdit.yaml*`）：から入力を読み取るときに値を作成または変更する必要があるテストケース `variable.yaml` ファイル。
...
** `*lunAdd.yaml*` （または `*lunEdit.yaml*`）：から入力を読み取るときに値を作成または変更する必要があるテストケース `variable.yaml` ファイル。
