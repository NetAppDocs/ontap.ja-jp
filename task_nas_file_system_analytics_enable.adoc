...
.手順
ボリュームのサイズと内容によっては、 ONTAP がボリューム内の既存データを処理する間、分析を有効にするのに時間がかかることがあります。完了すると、 System Manager に進捗状況が表示され、分析データが表示されます。初期化の進捗状況の詳細については、ONTAP のCLIコマンドを使用してください `volume analytics show`。
...
. 次のコマンドを実行します。
`volume analytics on -vserver _svm_name_ -volume _volume_name_ [-foreground {true|false}]`デフォルトでは、このコマンドはフォアグラウンドで実行されます。 ONTAP は進捗状況を表示し、完了すると分析データを表示します。より詳細な情報が必要な場合は、を使用して、コマンドをバックグラウンドで実行できます `-foreground false` オプションを選択し、を使用します `volume analytics show` CLIでの初期化の進捗状況を表示するコマンド。
