...
+
アクセスの検証が完了したらlink:task_nas_provision_export_policies.html["ボリュームのエクスポートポリシーを使用してクライアントアクセスを制限し、共有 ACL を使用してクライアントアクセスを制限します"]、エクスポートおよび共有ボリュームに所有権と権限を設定できます。
