...
この手順は、で有効になっている既存の Storage VM に Kerberos を設定します link:task_nas_enable_linux_nfs.html["NFS"] または 。
...
+
.. Storage VM のルートボリュームについて、関連する権限を表示します。 volume show -volume _root_vol_name __ -fields user 、 group 、 unix-permissions
+
...
+
.. ローカル UNIX ユーザを表示します。 vserver services name-service unix-user show -vserver vserver_name _`
+
...
.. これらの値が表示されない場合は、「 vserver services name-service unix-user modify 」コマンドを使用して値を更新します。


. Storage VM ルートボリュームに対するグループ権限を設定します。
+
.. ローカル UNIX グループを表示します。 vserver services name-service unix-group show -vserver vserver_name _`
+
...
| ルート | 0 
|===
.. これらの値が表示されていない場合は、「 vserver services name-service unix-group modify 」コマンドを使用して、値を更新します。


. Kerberos を設定するには、 System Manager に切り替えてください
. ONTAP システムマネージャで、 * Storage > Storage VM* をクリックし、 Storage VM を選択します。
. [* 設定 * ] をクリックします。
