...
この手順は、既存の Storage VM に新しいネームスペースを作成します。Storage VM で NVMe を設定し、 FC 転送がすでにセットアップされている必要があります。

image:workflow_nvme_provision_suse_linux.gif["SUSE Linux 用の NVMe をプロビジョニングするワークフロー図"]
...
. ネームスペースを初期化し、必要に応じてファイルシステムでフォーマットします。
