...
[role="lead"]
自動無停止アップグレード（ANDU）を実行すると、ONTAP は各ノードにターゲット ONTAP イメージを自動的にインストールし、クラスターが正常にアップグレードできることを検証してから、クラスター内のノード数に基づいてバックグラウンドで xref:concept_upgrade_methods.html[バッチ アップグレードまたはローリング アップグレード] を実行します。
...
IMPORTANT: ANDU の開始前に `storage failover modify-auto-giveback`コマンドオプションの設定を変更しても、アップグレードプロセスには影響しません。ANDU プロセスは、更新に必要なテイクオーバー/ギブバック中に、このオプションに設定されているプリセット値を無視します。たとえば、ANDU を開始する前に `-autogiveback`を false に設定しても、ギブバック前に自動アップグレードが中断されることはありません。 `storage failover modify-auto-giveback`の詳細については、link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=storage+failover+modify-auto-giveback["ONTAPコマンド リファレンス"^]を参照してください。
...
* オプションで、link:../update/pre-stage-system-firmware.html["シングルリブートアップグレード用のシステムファームウェアの事前準備"]。
+
ONTAP 9.17.1 以降、ANDU を実行する前にノードのブートデバイスに新しい BIOS または LOADER ファームウェアが既に存在する場合、アップグレードプロセスは ONTAP イメージにバンドルされているバージョンで上書きするのではなく、新しいファームウェアを保持します。これにより、ONTAP ソフトウェアのアップグレードとファームウェアのアップデートを 1 回の再起動サイクルで完了できます。
...
+
anonymous FTPの場合、URLは\ftp://anonymous@ftpserverの形式で指定する必要があります。
...
|===
.. *検証*を選択して、アップグレード前の検証チェックを実行します。
+
...
+
完了した自動アップグレード前チェックのリストが、アップグレード プロセスを開始する前に対処しておくべきエラーや警告とともに表示されます。アップグレードに進む前に、すべてのエラーを解決する必要があります。警告も解決しておくことが推奨されます。
...
====
.次の手順
ファームウェアを事前にステージングした場合は、 `system node firmware bootmedia show`コマンドを使用して、各ノードで目的のファームウェアバージョンがアクティブになっていることを確認してください。
