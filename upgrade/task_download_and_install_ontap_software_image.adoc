...
ONTAP 9.6 以降では、正しい Web サーバ SSL 証明書がストレージ側にインストールされている場合、 HTTPS オプションを使用できます
...
. イメージをダウンロードします。
+
ONTAP 9.6 以降では、ストレージに正しい Web サーバ SSL 証明書がインストールされている場合にオプションとして HTTPS を使用できます。
