...
. キューに格納されているアグリゲート、ボリューム、またはSnapshotコピーに関するジョブを削除します。 `job delete -id job_id`
+
[listing]
----
...
----
. 必要に応じて、特定した各CIFSセッションで開いているファイルを確認します。 `vserver cifs session file show -session-id session_ID`
+
[listing]
----
