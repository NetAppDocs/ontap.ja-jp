...
---
= 4ノードまたは8ノードの MetroCluster 構成での手動 ONTAP NDU
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
4ノードまたは8ノードのMetroCluster構成の手動による無停止アップグレード（NDU）には、アップデートの準備、1つまたは2つのDRグループそれぞれのDRペアの同時アップデート、およびアップグレード後のタスクの実行が含まれます。

.タスク概要
* このタスクを実行するには、ONTAP コマンドラインインターフェース（CLI）を使用する必要があります。
* このタスクは、次の構成に該当します。
+
...
----
+
次の例は、システム イメージ 2（ターゲット ONTAP イメージ）が node_A_1 のデフォルトおよび現在のバージョンであることを示しています：
...
----
+
次の例は、システム イメージ 2（ターゲット ONTAP イメージ）が node_B_1 のデフォルトおよび現在のバージョンであることを示しています。
...
----
+
次の例は、システム イメージ 2（ターゲット ONTAP イメージ）が node_A_2 のデフォルトおよび現在のバージョンであることを示しています：
...
----
+
次の例は、システム イメージ 2（ターゲット ONTAP イメージ）が node_B_2 のデフォルトおよび現在のバージョンであることを示しています。
