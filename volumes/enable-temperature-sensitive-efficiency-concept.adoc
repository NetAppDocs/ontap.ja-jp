...
TSSE はONTAP 9.8 で導入され、新しく作成されたシンプロビジョニングされたAFFボリュームで自動的に有効になります。既存のシンプロビジョニングされたAFFボリュームとシンプロビジョニングされた非AFF DP ボリュームで、温度に敏感なストレージ効率を有効にすることができます。TSSE はシックプロビジョニングされたボリュームではサポートされません。
...
温度に敏感なストレージ効率は、 AFF C シリーズ プラットフォームではデフォルトで有効になっています。また、ボリューム移動またはSnapMirrorを使用して、移行先に次のリリースがインストールされた状態で、非 TSSE プラットフォームから TSSE 対応の C シリーズ プラットフォームにシン プロビジョニングされたボリュームを移行する場合にも有効になっています。
...
既存のシンプロビジョニングボリュームでは、温度に敏感なストレージ効率は自動的には有効になりませんが、 link:../volumes/change-efficiency-mode-task.html["Storage Efficiencyモードを変更します"] 手動で効率的なモードに変更します。
